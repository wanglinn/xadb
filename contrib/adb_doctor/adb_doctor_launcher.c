/*-------------------------------------------
 *
 * adb_doctor_launcher.c 
 *
 * launch and control doctor processes
 *-------------------------------------------
 */
#include "postgres.h"
#include "pgstat.h"
...
/* create a shm with size segsize, insert data into this shm, then return these things. */
static AdbDoctorBgworkerDataShm *initializeBgworkerDataShm(AdbDoctorBgworkerData *data,
                                                           Size segsize,
                                                           Size datasize,
                                                           uint64 *tocKey);
/* setup a single shm for each doctor process, used to transfer data to it */
static AdbDoctorBgworkerDataShm *setupNodeDataShm(AdbDoctorNodeData *nodeData);
/* setup a single shm for host monitor doctor process, used to transfer data to it */
static AdbDoctorBgworkerDataShm *setupHostDataShm(AdbDoctorHostData *hostData);
/* setup a single shm for switcher doctor process, used to transfer data to it */
static AdbDoctorBgworkerDataShm *setupSwitcherDataShm(AdbDoctorSwitcherData *switcherData);
/* setup a single shm for each doctor process, used to transfer data to it */
static AdbDoctorBgworkerDataShm *setupDataShm(AdbDoctorBgworkerStatus *bgworkerStatus);
...
/* terminate functions */
static void terminateAllDoctor();
static void terminateDoctorList(AdbDoctorList *dataList);
static void terminateDoctorByName(char *name);
static void terminateDoctor(AdbDoctorBgworkerStatus *bgworkerStatus, bool waitFor);

/* check the doctor process running status */
static void checkDoctorRunningStatus();
...
static AdbDoctorList *getStaleBgworkerDataList();
...
/* 
 * when signaled, check the status of doctor processes, check the conf and (mgr_node,mgr_host) data change.
 ...
 * if change some data's field, we terminate the old doctor process and then startup a new doctor process.
 */
static void launcherLoop(AdbDoctorConf *conf)
{
    int rc;
    ...
    while (!gotSigterm)
    {
        ...
        confChanged = compareAndUpdateAdbDoctorConf(staleConf, freshConf);
...
static char *getDoctorName(AdbDoctorBgworkerData *data)
{
    ...
    {
        name = psprintf("adb doctor node monitor %u", ((AdbDoctorNodeData *)data)->wrapper->oid);
    }
    ...
    else
    {
        name = NULL;
        ereport(ERROR,
                (errmsg("could not recognize type:%d", data->type)));
    }
    return name;
}
 ...
 * create a shm_toc for convenience to insert data into shm.
 * memcpy data with size datasize (according to actual size) of data into this shm with the tocKey,
...
static AdbDoctorBgworkerDataShm *setupNodeDataShm(AdbDoctorNodeData *nodeData)
{
    ...
    shm_toc_estimate_chunk(&e, strlen(nodeData->wrapper->nodepath) + 1); /* need the tail \0 */
    nkeys++;

    shm_toc_estimate_chunk(&e, strlen(nodeData->wrapper->hostaddr) + 1);
    nkeys++;

    shm_toc_estimate_keys(&e, nkeys);
    segsize = shm_toc_estimate(&e);

    dataShm = initializeBgworkerDataShm((AdbDoctorBgworkerData *)nodeData, segsize, datasize, &tocKey);
    ...
    nodeDataInShm->wrapper->nodepath = shm_toc_allocate(toc, strlen(nodeData->wrapper->nodepath) + 1);
    strcpy(nodeDataInShm->wrapper->nodepath, nodeData->wrapper->nodepath);
    shm_toc_insert(toc, tocKey++, nodeDataInShm->wrapper->nodepath);

    nodeDataInShm->wrapper->hostaddr = shm_toc_allocate(toc, strlen(nodeData->wrapper->hostaddr) + 1);
...
static AdbDoctorBgworkerDataShm *setupHostDataShm(AdbDoctorHostData *hostData)
{
    ...
    dlist_foreach(iter, &hostData->list->head)
    {
        ...
        shm_toc_estimate_chunk(&e, strlen(hostWrapper->hostaddr) + 1);
        nkeys++;

        shm_toc_estimate_chunk(&e, strlen(hostWrapper->hostadbhome) + 1);
        nkeys++;
    }
    ...
    dataShm = initializeBgworkerDataShm((AdbDoctorBgworkerData *)hostData, segsize, datasize, &tocKey);
    ...
    dlist_foreach(iter, &hostData->list->head)
    {
        ...
        hostWrapperInShm->hostaddr = shm_toc_allocate(toc, strlen(hostWrapper->hostaddr) + 1);
        shm_toc_insert(toc, tocKey++, hostWrapperInShm->hostaddr);
        strcpy(hostWrapperInShm->hostaddr, hostWrapper->hostaddr);

        hostWrapperInShm->hostadbhome = shm_toc_allocate(toc, strlen(hostWrapper->hostadbhome) + 1);
...
static AdbDoctorBgworkerDataShm *setupSwitcherDataShm(AdbDoctorSwitcherData *switcherData)
{
    ...
    shm_toc_estimate_chunk(&e, strlen(switcherData->wrapper->nodepath) + 1); /* need the tail \0 */
    nkeys++;

    shm_toc_estimate_chunk(&e, strlen(switcherData->wrapper->hostaddr) + 1);
    nkeys++;

    shm_toc_estimate_keys(&e, nkeys);
    segsize = shm_toc_estimate(&e);

    dataShm = initializeBgworkerDataShm((AdbDoctorBgworkerData *)switcherData, segsize, datasize, &tocKey);
    ...
    switcherDataInShm->wrapper->nodepath = shm_toc_allocate(toc, strlen(switcherData->wrapper->nodepath) + 1);
    ...
    switcherDataInShm->wrapper->hostaddr = shm_toc_allocate(toc, strlen(switcherData->wrapper->hostaddr) + 1);
 ...
 * set bgw_start_time=BgWorkerStart_RecoveryFinished (start as soon as the system has entered normal read-write state)
 * set bgw_restart_time=BGW_NEVER_RESTART, indicating not to restart the process in case of a crash,
 * If it collapsed, it doesn't matter, the launcher will restart it if necessary.
 */
static void registerDoctorAsBgworker(AdbDoctorBgworkerStatus *bgworkerStatus, AdbDoctorBgworkerDataShm *dataShm)
{
    Adb_Doctor_Bgworker_Type type;
                                     ...
                                     volatile AdbDoctorBgworkerData *dataInShm)
{
    ...
    status = WaitForBackgroundWorkerStartup(bgworkerStatus->handle, &bgworkerStatus->pid);
    ...
    /* the BGWH_STARTED can not indicate that it is working properly, should wait for the variable "ready" in shm be set to true */
    for (;;)
    {
        ...
        status = GetBackgroundWorkerPid(bgworkerStatus->handle, &bgworkerStatus->pid);
        if (status != BGWH_STARTED)
            ereport(ERROR,
                    (errcode(ERRCODE_INSUFFICIENT_RESOURCES),
                     errmsg("could not start background process"),
 ...
 * the private global variable bgworkerStatusList when a doctor process launched.
 * with the help of bgworkerStatusList, we can view or control doctor process status as we want.
 */
static void launchDoctorList(AdbDoctorList *dataList)
{
    AdbDoctorLink *link;
...
static AdbDoctorBgworkerStatus *launchDoctor(AdbDoctorBgworkerData *data)
{
    ...
    /* if launcher exits abnormally, the shm will be detached, the worker launched should exit too. */
    on_dsm_detach(dataShm->seg, cleanupAdbDoctorBgworker, PointerGetDatum(bgworkerStatus->handle));

    waitForDoctorBecomeReady(bgworkerStatus, (AdbDoctorBgworkerData *)dataShm->dataInShm);
    /* the doctor process become ready, the shm should be detached, this time do not let the doctor process exit. */
    cancel_on_dsm_detach(dataShm->seg, cleanupAdbDoctorBgworker, PointerGetDatum(bgworkerStatus->handle));
...
static void terminateAllDoctor()
{
    ...
    {
        bgworkerStatus = dlist_container(AdbDoctorBgworkerStatus, wi_links, miter.cur);
...
static void terminateDoctor(AdbDoctorBgworkerStatus *bgworkerStatus, bool waitFor)
{
    ...
    if (waitFor)
    {
        ...
        if (status != BGWH_STOPPED)
            ereport(ERROR,
                    (errmsg("could not terminate background process, the postmaster may died.")));
    }
}
...
static void signalDoctorByName(char *name)
{
    ...
    {
        bgworkerStatus = dlist_container(AdbDoctorBgworkerStatus, wi_links, iter.cur);
 ...
 */
static void queryBgworkerDataAndConf(AdbDoctorConf **confP, AdbDoctorList **dataListP)
{
    ...
    /* query out all configuration values from table adb_doctor_conf */
    SPI_selectAllConfValue(confP, oldContext);
...
/**
 * backend create a dsm_segment, and transfer a variable dsm_handle to here by BackgroundWorker mechanism,
 * use variable dsm_handle to attach the dsm_segment, and then get the shm_mq that backend have set.
 * when all doctor processes launched ok, we can send message "OK" to backend by this shm_mq.
 */
void attachShm(Datum main_arg, bool *needNotify, dsm_segment **segP, shm_mq_handle **mqhP)
{
    if (main_arg == 0)
    {
    ...
    if (seg == NULL)
    {
        *needNotify = false;
        ereport(LOG,
                (errmsg("unable to map dynamic shared memory segment, ignore it.")));
    }
    else
    {
        *needNotify = true;
        ...
        *segP = seg;
        shm_toc *toc = shm_toc_attach(ADB_DOCTOR_LAUNCHER_MAGIC, dsm_segment_address(seg));
        if (toc == NULL)
            ereport(ERROR,
                    (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                     errmsg("bad magic number in dynamic shared memory segment")));
        shm_mq *mq = shm_toc_lookup(toc, 0, false);
 ...
 * refresh doctor processes (include terminate,launch,signal etc) if necessary, or do nothing.
 * the so called BgworkerData is the data in tables mgr_node and mgr_host,
                                    ...
                                    AdbDoctorList *freshDataList)
{
    ...
    logAdbDoctorBgworkerDataList(addedDataList, "addedDataList", LOG);
    logAdbDoctorBgworkerDataList(deletedDataList, "deletedDataList", LOG);
    logAdbDoctorBgworkerDataList(changedDataList, "changedDataList", LOG);
    logAdbDoctorBgworkerDataList(identicalDataList, "identicalDataList", LOG);
    ...
    /* bgworkerStatusList would not link any data in identicalDataList, so pfree it and data in it. */
 ...
 * the recomended way is release the memory except the AdbDoctorMgrNodeData linked in the AdbDoctorList, see below:
 * pfreeAdbDoctorList(list, false);
 * pfreeAdbDoctorList(changedDataList, false);
 * pfreeAdbDoctorList(identicalDataList, true);
 * pfreeAdbDoctorList(deletedDataList, false);
 * why? because AdbDoctorBgworkerData from fresh and stale list only has one copy, if you free them, may take wrong.
 */
static void compareBgworkerDataList(AdbDoctorList *staleDataList,
                                    AdbDoctorList *freshDataList,
                                    AdbDoctorList **addedDataListP,
                                    ...
                                    AdbDoctorList **identicalDataListP)
{
    ...
    /*
     * get information of added,deleted,changed through comparing the fresh and stale data(mgr_node,mgr_host).
     * when all of this comparison done, the above lists can be used to do some thing about control doctor process.
     */
