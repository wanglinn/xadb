						  ...
						  bool omitSavePlan)
{
	...
	slot = table_slot_create(adbssRel, NULL);
	...
	found = false;
	while (index_getnext_slot(queryidIndexScanDesc, ForwardScanDirection, slot))
	{
		value = slot_getattr(slot, Anum_adbss_userid, &isnull);
		if (isnull || DatumGetObjectId(value) != adbssKey->userid)
			continue;
		value = slot_getattr(slot, Anum_adbss_dbid, &isnull);
		if (isnull || DatumGetObjectId(value) != adbssKey->dbid)
			continue;
		value = slot_getattr(slot, Anum_adbss_planid, &isnull);
		if (isnull)
			continue;
		...
		if (storedPlanid == adbssKey->planid)
		{
			if (omitSavePlan)
			{
				// identical plan
				found = true;
				*foundTuple = ExecCopySlotHeapTuple(slot);
				break;
			}
			else
			{
				value = slot_getattr(slot, Anum_adbss_plan, &isnull);
				if (isnull)
					continue;
...
{
	TupleDesc textTupleDesc;
	textTupleDesc = CreateTemplateTupleDesc(Natts_adbss - Anum_adbss_offset);
	...
	while ((entry = hash_seq_search(&hash_seq)) != NULL)
	{
		...
		if (is_allowed_role || entry->key.userid == userid)
		{
			...
			if (showtext && entry->tupleInDsa != InvalidDsaPointer)
			{
				...
				slot = MakeSingleTupleTableSlot(textTupleDesc, &TTSOpsMinimalTuple);
