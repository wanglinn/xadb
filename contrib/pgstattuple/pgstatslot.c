...
pgstatdatabaseslot(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < SPI_processed; i++)
	{
		...
		if(0==strcmp(pschema, "information_schema"))
			continue;
	...
	foreach (lc, tablename_list)
	{
		...
		if (LOCATOR_TYPE_HASHMAP != rel->rd_locator_info->locatorType)
...
pgstat_heap_slot(Relation rel, FunctionCallInfo fcinfo, pgstattuple_type *pstat, BlockNumber* pallnblocks)
{
	...
	while ((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		...
		if (HeapTupleSatisfiesVisibility(tuple, snapshot, scan->rs_cbuf))
		{
...
#define NCHARS_SLOT		32
	...
	for(row_id=0; row_id<HASHMAP_SLOTSIZE; row_id++)
	{
		...
		slot_located = get_pgxc_nodename(nodeindex);
...
pgvalueslot(PG_FUNCTION_ARGS)
{
	...
	if (LOCATOR_TYPE_HASHMAP != locatorType)
