...
ReceiveTarAndUnpackCopyChunk(size_t r, char *copybuf, void *callback_data)
{
	...
	if (state->file == NULL)
	{
		 ...
		 */
#ifdef ADB
		if (strlen(copybuf) > (sizeof(TABLESPACE_VERSION_DIRECTORY) - 1)
			&& strncmp(copybuf, TABLESPACE_VERSION_DIRECTORY, sizeof(TABLESPACE_VERSION_DIRECTORY) - 1) ==0)
		{
			char *newpostion = copybuf;
			newpostion = newpostion + sizeof(char)*(sizeof(TABLESPACE_VERSION_DIRECTORY) - 1);
			while(*newpostion != '\0')
			{
				if (*newpostion == '/')
					break;
...
main(int argc, char **argv)
{
	...
	if (argc > 1)
	{
		...
		{
#ifdef ADB
			puts("pg_basebackup (" ADB_VERSION " based on PostgreSQL) " PG_VERSION);
#else
