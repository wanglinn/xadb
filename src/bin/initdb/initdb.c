...
readfile(const char *path)
{
	...
	{
#ifdef ADB
		if(strncmp(line.data, "--ADBONLY", 9) == 0)
			memset(line.data, ' ', 9);;
#elif defined(INITMGR)
		if(strncmp(line.data, "--MGRONLY", 9) == 0)
			memset(line.data, ' ', 9);
#endif
...
#define BINTARGET "postgres"
	...
	if ((ret = find_other_exec(argv0, BINTARGET, PG_BACKEND_VERSIONSTR,
							   backend_exec)) < 0)
	{
		...
		if (ret == -1)
			pg_log_error("The program \"%s\" is needed by %s but was not found in the\n"
						 ...
						 BINTARGET, progname, full_path);
		else
			pg_log_error("The program \"%s\" was found by \"%s\"\n"
						 "but was not the same version as %s.\n"
						 "Check your installation.",
						 BINTARGET, full_path, progname);
		exit(1);
	}
#undef BINTARGET
...
#warning TODO system_constraints.sql and system_functions.sql
#else /* INITMGR */
...
#ifdef ADB
		{"nodename", required_argument, NULL, 14},
#endif
		{NULL, 0, NULL, 0}
	};
