...
main(int argc, char **argv)
{
...
#ifdef WITH_RDMA
	while ((c = getopt_long(argc, argv, "abBcCd:e:E:f:F:h:j:n:N:Op:RrsS:t:T:U:vwWxZ:",
	 ...
	 * death.
	 */
	ConnectDatabase(fout, &dopt.cparams, false ADB_RDMA_COMMA_ARG(dopt.is_rs));
#ifdef MGR_DUMP
	if (!adbmgr_table)
#endif
				...
				if (strlen(distribute_def) > 0)
				{
					...
					else
					{
							...
							tbinfo->pgxclocatortype == 'L' )
						{
							...
							pg_log_error("Dump the table defined by 'DISTRIBUTE BY RANGE or LIST' must include 'TO NODE' clause, please see Option --include-nodes.\n");
							appendPQExpBuffer(q, "\n%s", distribute_def);
						}
						else
						{
...
dumpSequenceData(Archive *fout, const TableDataInfo *tdinfo)
{
	...
	if (PQntuples(res) != 1)
	{
		pg_log_error(ngettext("query to get nextval of sequence \"%s\" "
...
dumpAdbmgrTable(Archive *fout)
{
	...
	ArchiveEntry(fout, nilCatalogId, createDumpId(),
				 ARCHIVE_OPTS(.tag = "mgr_host",
							  .namespace = "pg_catalog",
							  .description = "DEFAULT",
							  .section = SECTION_NONE,
							  .createStmt = addstrdata->data));
	PQclear(res);
	...
	ArchiveEntry(fout, nilCatalogId, createDumpId(),
				 ARCHIVE_OPTS(.tag = "mgr_updateparm",
							  .namespace = "pg_catalog",
							  .description = "DEFAULT",
							  .section = SECTION_NONE,
							  .createStmt = addstrdata->data));
	...
	if (i > 0)
	{
		...
		if (PQntuples(res) > 0)
		{
			...
			ArchiveEntry(fout, nilCatalogId, createDumpId(),
						 ARCHIVE_OPTS(.tag = "adb_doctor_conf",
									  .namespace = "adb_doctor",
									  .description = "DEFAULT",
									  .section = SECTION_DATA,
									  .createStmt = addstrdata->data));
		}
