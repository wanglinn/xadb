...
#ifdef ADB
	{"BARRIER", NULL, NULL, NULL},	/* Comes barrier name next, so skip it */
#endif
...
#ifdef ADB
#define ADB_ONLY_MEB_COMM(v) v,
#else
#define ADB_ONLY_MEB_COMM(v)
#endif /* ADB */

	/* Known command-starting keywords. */
	static const char *const sql_commands[] = {
		"ABORT", "ALTER", "ANALYZE", "BEGIN", "CALL", "CHECKPOINT", ADB_ONLY_MEB_COMM("CLEAN CONNECTION") "CLOSE", "CLUSTER",
		"COMMENT", "COMMIT", "COPY", "CREATE", "DEALLOCATE", "DECLARE",
		"DELETE FROM", "DISCARD", "DO", "DROP", "END", "EXECUTE", ADB_ONLY_MEB_COMM("EXECUTE DIRECT") "EXPLAIN",
...
#ifdef ADB
	/* ALTER NODE */
	else if (Matches("ALTER", "NODE"))
		COMPLETE_WITH_QUERY(Query_for_list_of_available_nodenames);
	else if (Matches("ALTER", "NODE", MatchAny))
		COMPLETE_WITH("WITH");
	else if (Matches("ALTER", "NODE", MatchAny, "WITH"))
		COMPLETE_WITH("(");
	else if (Matches("ALTER", "NODE", MatchAny, "WITH", MatchAny))
	{
...
/* CLEAN */
	...
	/* CLEAN CONNECTION */
	else if (Matches("CLEAN", "CONNECTION"))
		COMPLETE_WITH("TO");
	/* CLEAN CONNECTION TO */
	else if (Matches("CLEAN", "CONNECTION", "TO"))
		COMPLETE_WITH("ALL", "COORDINATOR", "NODE");
	/* CLEAN CONNECTION TO ALL */
	else if (Matches("CLEAN", "CONNECTION", "TO", "ALL"))
		COMPLETE_WITH("FOR DATABASE", "TO USER", "FORCE");
	/* CLEAN CONNECTION TO ALL FORCE */
	else if (Matches("CLEAN", "CONNECTION", "TO", "ALL", "FORCE"))
		COMPLETE_WITH("FOR DATABASE", "TO USER");
	/* CLEAN CONNECTION TO ALL [FORCE] FOR */
	else if (Matches("CLEAN", "CONNECTION", "TO", "ALL", "FORCE", "FOR") ||
			 Matches("CLEAN", "CONNECTION", "TO", "ALL", "FOR"))
		COMPLETE_WITH("DATABASE");
	/* CLEAN CONNECTION TO ALL [FORCE] FOR DATABASE */
	else if (Matches("CLEAN", "CONNECTION", "TO", "ALL", "FORCE", "FOR", "DATABASE") ||
			 Matches("CLEAN", "CONNECTION", "TO", "ALL", "FOR", "DATABASE"))
		COMPLETE_WITH_QUERY(Query_for_list_of_databases);
	/* CLEAN CONNECTION TO ALL [FORCE] FOR DATABASE name */
	else if (Matches("CLEAN", "CONNECTION", "TO", "ALL", "FORCE", "FOR", "DATABASE", MatchAny) ||
			 Matches("CLEAN", "CONNECTION", "TO", "ALL", "FOR", "DATABASE", MatchAny))
		COMPLETE_WITH("TO USER");
	/* ... FOR DATABASE name TO USER */
	else if (TailMatches("FOR", "DATABASE", MatchAny, "TO", "USER"))
		COMPLETE_WITH_QUERY(Query_for_list_of_roles);
	/* CLEAN CONNECTION TO ALL [FORCE] TO USER */
	else if (Matches("CLEAN", "CONNECTION", "TO", "ALL", "FORCE", "TO", "USER") ||
			 Matches("CLEAN", "CONNECTION", "TO", "ALL", "TO", "USER"))
		COMPLETE_WITH_QUERY(Query_for_list_of_roles);
	/* CLEAN CONNECTION TO COORDINATOR */
	else if (Matches("CLEAN", "CONNECTION", "TO", "COORDINATOR"))
		COMPLETE_WITH("(");
	/* CLEAN CONNECTION TO COORDINATOR ( */
	else if (Matches("CLEAN", "CONNECTION", "TO", "COORDINATOR", "("))
		COMPLETE_WITH(Query_for_list_of_available_coordinators);
	/* CLEAN CONNECTION TO NODE */
	else if (Matches("CLEAN", "CONNECTION", "TO", "NODE"))
		COMPLETE_WITH("(");
	/* CLEAN CONNECTION TO NODE ( */
	else if (Matches("CLEAN", "CONNECTION", "TO", "NODE", "("))
...
#ifdef ADB
	/* CREATE NODE */
	else if (Matches("CREATE", "NODE", MatchAny))
		COMPLETE_WITH("WITH");
	else if (Matches("CREATE", "NODE", MatchAny, "WITH"))
		COMPLETE_WITH("(");
	else if (Matches("CREATE", "NODE", MatchAny, "WITH", MatchAny))
		COMPLETE_WITH("TYPE", "HOST", "PORT", "PRIMARY", "PREFERRED");
	/* CREATE NODEGROUP */
	else if (Matches("CREATE", "NODE", "GROUP", MatchAny))
		COMPLETE_WITH("WITH");
#endif
...
/* DROP */
	...
	/* DROP NODE GROUP */
	else if (Matches("DROP", "NODE", "GROUP"))
		COMPLETE_WITH_QUERY(Query_for_list_of_available_nodegroup_names);
/* EXECUTE DIRECT */
	else if (Matches("EXECUTE", "DIRECT"))
		COMPLETE_WITH("ON");
	else if (Matches("EXECUTE", "DIRECT", "ON"))
		COMPLETE_WITH_QUERY(Query_for_list_of_available_nodenames);
#endif
