...
#ifdef ADB
	printf(_("  -N, --nodename=NAME    set current node's name for ADB\n"));
#endif
...
main(int argc, char **argv)
{
	...
	if (argc > 1)
	{
		...
		{
#ifdef ADB
			puts("adb_waldump (" ADB_VERSION " based on PostgreSQL) " PG_VERSION);
#else
	...
	while ((option = getopt_long(argc, argv, "be:fn:p:qr:s:t:x:z" ADB_ONLY_CODE("N"),
