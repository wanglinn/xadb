...
select 1+to_char(1) from dual;
create table tt(id number,name varchar2(3),ad char(3));
insert into tt values(1,'1.0','1');
select 1+name from tt;
select 1+ad from tt;
drop table tt;
...
select * from tt where id > all(select id from aa);
drop table tt;
drop table aa;
--between and		
