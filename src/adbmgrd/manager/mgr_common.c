...
TupleDesc get_list_nodesize_tuple_desc(void)
{
	if(common_list_nodesize == NULL)
	{
		...
		PG_TRY();
		{
			desc = CreateTemplateTupleDesc(5);
...
TupleDesc get_ha_replication_tuple_desc(void)
{
	if(ha_replication_tuple_desc == NULL)
	{
		...
		PG_TRY();
		{
			desc = CreateTemplateTupleDesc(12);
...
bool mgr_rewind_node(char nodetype, char *nodename, StringInfo strinfo)
{
	...
	while((node_tuple = heap_getnext(rel_scan, ForwardScanDirection)) != NULL)
	{
		...
		if (!(CNDN_TYPE_GTM_COOR_MASTER == mgr_node->nodetype || CNDN_TYPE_GTM_COOR_SLAVE == mgr_node->nodetype
				 || mgr_node->oid == master_nodeinfo.tupleoid || mgr_node->nodemasternameoid ==master_nodeinfo.tupleoid))
				continue;
...
int mgr_get_normal_slave_node(Relation relNode, Oid masterTupleOid, int sync_state_sync, Oid excludeOid, Name slaveNodeName)
{
	...
	relScan = table_beginscan_catalog(relNode, 5, key);
...
bool mgr_check_slave_replicate_status(const Oid masterTupleOid, const char nodetype, const char *slaveName)
{
	...
	Assert(mgr_mhost);
	nodeRel = table_open(HostRelationId, AccessShareLock);
...
HeapTuple mgr_get_sync_slavenode_tuple(Oid mastertupleoid, bool bincluster, Oid includeOid, Oid excludeoid, int seqNum)
{
	...
	while((tuple = heap_getnext(relScan, ForwardScanDirection)) != NULL)
	{
		...
		if (mgr_node->oid == excludeoid)
			continue;
		else if (mgr_node->oid == includeOid)
...
bool mgr_get_createnodeCmd_on_readonly_cn(char *nodeName, bool bincluster, StringInfo cmdstring)
{
	...
	*/
	...
	while((tuple = heap_getnext(relScan, ForwardScanDirection)) != NULL)
	{
		...
		syncNodeTuple = mgr_get_sync_slavenode_tuple(mgr_node->oid, bincluster, InvalidOid, InvalidOid, seqNum);
	...
	while((tuple = heap_getnext(relScan, ForwardScanDirection)) != NULL)
	{
		...
		foreach(dnCeil, dnList)
		{
			...
			if (mgr_check_list_in(dnPreferredList, nodename))
				continue;
			if (mgr_node->oid == mgr_get_tupoid_from_nodename(relNode, nodename))
			{
	...
	foreach(dnCeil, dnList)
	{
		...
		while((tuple = heap_getnext(relScan, ForwardScanDirection)) != NULL)
		{
			...
			if (mgr_node->nodetype == CNDN_TYPE_DATANODE_MASTER)
			{
				...
				syncNodeTuple = mgr_get_sync_slavenode_tuple(mgr_node->oid
...
void check_zone_node_incluster(char *zone)
{
	...
	ScanKeyInit(&key[0]
			...
			,CStringGetDatum(zone));
	relNode = table_open(NodeRelationId, AccessShareLock);
	scan = table_beginscan_catalog(relNode,1, key);
	while((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		mgr_node = (Form_mgr_node)GETSTRUCT(tuple);
		Assert(mgr_node);
...
char* mgr_get_cmdname(int cmdtype)
{
	...
	static CmdTypeName cmds[] = {
		{AGT_CMD_GTMCOORD_INIT, 				"INIT GTMCOORD MASTER"},
		{AGT_CMD_GTMCOORD_SLAVE_INIT, 			"INIT GTMCOORD SLAVE "},
		{AGT_CMD_GTMCOORD_START_MASTER,			"START GTMCOORD MASTER"},
		{AGT_CMD_GTMCOORD_START_SLAVE,			"START GTMCOORD SLAVE"},	
		{AGT_CMD_GTMCOORD_STOP_MASTER,			"STOP GTMCOORD MASTER"},
		{AGT_CMD_GTMCOORD_STOP_SLAVE,			"STOP GTMCOORD SLAVE"},
		{AGT_CMD_GTMCOORD_CLEAN,				"CLEAN GTMCOORD"},
		{AGT_CMD_GTMCOORD_SLAVE_FAILOVER,		"FAILOVER GTMCOORD SLAVE"},
		{AGT_CMD_AGTM_RESTART,					"RESTART AGTM"},
		{AGT_CMD_CNDN_CNDN_INIT,				"INIT CNDN MASTER"},
		{AGT_CMD_CNDN_SLAVE_INIT,				"INIT CNDN SLAVE"},
		{AGT_CMD_CN_RESTART,					"RESTART COORD"},
		{AGT_CMD_CN_START,						"START COORD"},
		{AGT_CMD_CN_STOP,						"STOP COORD"},
		{AGT_CMD_DN_START,						"START DATANODE"},
		{AGT_CMD_DN_RESTART,					"RESTART DATANODE"},
		{AGT_CMD_DN_STOP,						"STOP DATANODE"},
		{AGT_CMD_DN_FAILOVER,					"FAILOVER DATANODE"},	
		{AGT_CMD_PSQL_CMD,						"PSQL CMD"},
		...
		{AGT_CMD_CNDN_REFRESH_PGSQLCONF_RELOAD,	"REFRESH RELOAD CNDN PGSQLCONF"},
		{AGT_CMD_CNDN_DELPARAM_PGSQLCONF_FORCE,	"DELPARAM CNDN PGSQLCONF FORCE"},
		{AGT_CMD_CNDN_RENAME_RECOVERCONF,		"RENAME CNDN RECOVERCONF"},
		{AGT_CMD_MONITOR_GETS_HOST_INFO,		"GET HOST INFO"},
		{AGT_CMD_PGDUMPALL,						"PGDUMPALL"},
		{AGT_CMD_RM,							"REMOVE TEMP FILE"},
		{AGT_CMD_NODE_RELOAD,					"RELOAD NODE"},
		{AGT_CMD_CLEAN_NODE,					"CLEAN NODE"},
		{AGT_CMD_STOP_AGENT,					"STOP AGENT"},
		{AGT_CMD_SHOW_AGTM_PARAM,				"SHOW AGTM PARAM"},
		{AGT_CMD_SHOW_CNDN_PARAM,				"SHOW CNDN PARAM"},
		{AGT_CMD_GET_SQL_STRINGVALUES,			"GET SQL STRING VALUES"},
		{AGT_CMD_GET_EXPLAIN_STRINGVALUES,		"GET EXPLAIN STRING VALUES"},
		{AGT_CMD_GET_BATCH_JOB,					"GET BATCH JOB"},	
		{AGT_CMD_CHECK_DIR_EXIST,				"CHECK DIR EXIST"},
		{AGT_CMD_PING_NODE,						"PING NODE"},
		{AGT_CMD_NODE_REWIND,					"REWIND NODE"},
		{AGT_CMD_GTMCOORD_START_MASTER_BACKEND,	"START GTMCOORD MASTER BACKEND"},
		{AGT_CMD_GTMCOORD_START_SLAVE_BACKEND,	"START GTMCOORD SLAVE BACKEND"},
		{AGT_CMD_CN_START_BACKEND,				"START COORD BACKEND"},
		{AGT_CMD_DN_START_BACKEND,				"START DATANODE BACKEND"},
		{AGT_CMD_GTMCOORD_STOP_MASTER_BACKEND,	"STOP GTMCOORD MASTER BACKEND"},
		{AGT_CMD_GTMCOORD_STOP_SLAVE_BACKEND,	"STOP GTMCOORD SLAVE BACKEND"},
		{AGT_CMD_CN_STOP_BACKEND,				"STOP COORD BACKEND"},
		{AGT_CMD_DN_STOP_BACKEND,				"STOP DATANODE BACKEND"},
		{AGT_CMD_GET_SQL_STRINGVALUES_COMMAND,	"GET SQL STRING VALUES COMMAND"},
		{AGT_CMD_AGTM_REWIND,					"REWIND AGTM"},
		{AGT_CMD_DN_MASTER_PROMOTE,				"PROMOTE DATANODE MASTER"},
		{AGT_CMD_CHECKOUT_NODE,					"CHECKOUT NODE"},
...
restore_node(char *nodepath)
{
	...
	while(fgets(rewindCmd, MAXPGPATH-1, rewind_file) != NULL)
	{
		...
		strlcpy(rewindNode->rewindcmd, rewindCmd, lengthof(rewindNode->rewindcmd));
