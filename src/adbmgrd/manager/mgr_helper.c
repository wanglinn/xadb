							   ...
							   MemoryContext spiContext)
{
	...
	nodeInDB = selectMgrNodeByOid(mgrNode->oid, spiContext);
	if (isMasterNode(nodeInDB->form.nodetype, true))
		nodemasternameoid = nodeInDB->oid;
	else
...
List *getNodeSyncConfigInRepGroup(MgrNodeWrapper *mgrNode)
{
	...
	PG_TRY();
	{
		...
		while ((tuple = heap_getnext(rel_scan, ForwardScanDirection)) != NULL)
		{
			...
			if (HeapTupleGetOid(tuple) == mgrNode->oid)
			{
				if (isMasterNode(temp_mgr_node->nodetype, true))
					nodemasternameoid = HeapTupleGetOid(tuple);
				else
		...
		{
			temp_mgr_node = (Form_mgr_node)GETSTRUCT(tuple);
			if ((HeapTupleGetOid(tuple) == nodemasternameoid ||
								 ...
								 StringInfoData *infosendmsg)
{
	...
	syncNum = mgr_get_master_sync_string(node->oid, true, node->oid, &infosendsyncmsg);
