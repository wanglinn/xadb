...
static bool pqAddTuple(PGresult *res, PGresAttValue *tup,
					   const char **errmsgp);
static int	PQsendQueryInternal(PGconn *conn, const char *query, bool newQuery ADB_ONLY_COMMA_ARG2(const char *query_tree, size_t tree_len));
#ifndef ADB
...
{
	return PQsendQueryInternal(conn, query, true ADB_ONLY_COMMA_ARG2(NULL, 0));
}

int
PQsendQueryContinue(PGconn *conn, const char *query)
{
	return PQsendQueryInternal(conn, query, false ADB_ONLY_COMMA_ARG2(NULL, 0));
}

static int
PQsendQueryInternal(PGconn *conn, const char *query, bool newQuery
					ADB_ONLY_COMMA_ARG2(const char *query_tree, size_t tree_len))
{
	...
	else
	{
		...
		{
			appendPQExpBufferStr(&conn->errorMessage,
								 libpq_gettext("send qeury tree can not call in pipeline mode"));
			goto sendFailed;
		}
#endif /* ADB */
		/*
...
{
	return PQsendQueryInternal(conn, query, true, query_tree, tree_len);
}
...
PQsendClose(PGconn *conn, int isStatement, const char *name)
{
	...
	{
		appendPQExpBufferStr(&conn->errorMessage,
							 libpq_gettext("send close can not call in pipeline mode"));
		return 0;
	}
						...
						const int *resultFormats)		/* For Bind message, "nResultFormat" of output parameter formats */
{
	...
	{
		appendPQExpBufferStr(&conn->errorMessage,
							 libpq_gettext("send qeury extend can not call in pipeline mode"));
		return 0;
	}
...
int pqSendAgtmListenPort(PGconn *conn, int port)
{
	...
	if (!PQexecStart(conn) || !PQsendQueryStart(conn, false))
		return -1;

	if (conn->pipelineStatus != PQ_PIPELINE_OFF)
	{
		appendPQExpBufferStr(&conn->errorMessage,
							 libpq_gettext("send AGTM listen port can not call in pipeline mode"));
		return -1;
	}
