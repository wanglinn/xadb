	...
	for (attrno = 1; attrno <= numattrs; attrno++)
	{
		...
		if (commandType == CMD_INSERT)
		{
			...
			if (att_tup->attgenerated && !apply_default ADB_ONLY_CODE(&& !IsConnFromCoord()))
			{
				 ...
				 */
				if (values_attrno != 0 ADB_ONLY_CODE(&& !IsConnFromCoord()))
				{
					if (default_only_cols == NULL)
						default_only_cols = findDefaultOnlyColumns(values_rte);
...
#warning droped function rewriteTargetListUD, see 86dc90056dfdbd9d1b891718d2e5614e3e432f35 March 31st, 2021 11:52pm
#endif /* ADB */
...
QueryRewrite(Query *parsetree)
{
	...
	if (parsetree->commandType == CMD_UTILITY)
	{
		if (IsA(parsetree->utilityStmt, CreateTableAsStmt) &&
			...
			((CreateTableAsStmt *)parsetree->utilityStmt)->objtype != OBJECT_MATVIEW)
		{
			/*
