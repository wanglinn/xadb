...
gram_parse_query(const char *query_string,
				 List *(parse_fun)(const char *, RawParseMode));
...
#ifdef ADB_MULTI_GRAM
	raw_parsetree_list = (*parse_fun)(query_string, RAW_PARSE_DEFAULT);
#else
...
#endif
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		plantree_list = pg_plan_queries(querytree_list, query_string,
										CURSOR_OPT_PARALLEL_OK ADB_ONLY_CODE(|CURSOR_OPT_CLUSTER_PLAN_SAFE), NULL);
...
exec_bind_message(StringInfo input_message)
{
	 ...
	 */
#ifdef ADB
	cplan = GetCachedClusterPlan(psrc, params, NULL, NULL);
#else
			 ...
			 const char *username)
{
	...
	if (sigsetjmp(local_sigjmp_buf, 1) != 0)
	{
		...
		PQNReleaseAllConnect(PQN_DEFAULT_TIMEOUT);
			...
			case 'L':		/* AGTM backend listen port */
				{
					...
					qc.commandTag = CMDTAG_AGTM_LISTEN_PORT;
					qc.datum = Int32GetDatum(listen_port);
					EndCommand(&qc, whereToSendOutput, false);
				}
				send_ready_for_query = true;
				break;
#endif /* ADB */
