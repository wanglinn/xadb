...
#ifdef ADB
#include "tcop/dest.h"	/* for COMPLETION_TAG_BUFSIZE */
#endif
...
#ifdef ADB
bool
command_tag_display_format(const QueryCompletion *qc, char *tag)
{
	switch(qc->commandTag)
	{
	case CMDTAG_AGTM_LISTEN_PORT:
		snprintf(tag, COMPLETION_TAG_BUFSIZE, "%d", DatumGetInt32(qc->datum));
		return true;
	case CMDTAG_BARRIER:
		snprintf(tag, COMPLETION_TAG_BUFSIZE, "%s %s",
				 "CLUSTER BARRIER",
				 DatumGetCString(qc->datum));
		return true;
	default:
		break;
	}
