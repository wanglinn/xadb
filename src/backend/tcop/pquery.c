...
#ifdef ADB
				QueryCompletion	local_qc;
				if (qc != NULL &&
					IS_PGXC_COORDINATOR &&
					pstmt->commandType == CMD_INSERT)
				{
					InitializeQueryCompletion(&local_qc);
					ProcessQuery(pstmt,
								 portal->sourceText,
								 portal->portalParams,
								 portal->queryEnv,
								 dest, &local_qc);
					if (likely(local_qc.commandTag == CMDTAG_INSERT))
					{
						qc->commandTag = CMDTAG_INSERT;
						qc->nprocessed += local_qc.nprocessed;
					}
				}else
#endif
				ProcessQuery(pstmt,
...
PlannedStmtRequiresSnapshot(PlannedStmt *pstmt)
{
	...
	/* efficiency hacks from here down */
		...
		(IsA(utilityStmt, CheckPointStmt) ADB_ONLY_CODE(&& IS_PGXC_DATANODE)))
		return false;
