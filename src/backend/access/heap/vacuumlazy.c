...
{
	TupleDesc desc = CreateTemplateTupleDesc(5);
...
{
	TupleTableSlot *slot = MakeSingleTupleTableSlot(desc, &TTSOpsMinimalTuple);
...
static TupleTableSlot* OnceTupleFunction(OnceTupleContext *context)
{
	if (context->slot == NULL)
	{
		TupleTableSlot *slot;
		int i;
		slot = context->slot = MakeSingleTupleTableSlot(context->desc, &TTSOpsMinimalTuple);
...
{
#ifdef ADB
	return lazy_vacuum_rel_ext(rel, params, bstrategy, NIL, NIL);
}
void
lazy_vacuum_rel_ext(Relation rel, VacuumParams *params,
					BufferAccessStrategy bstrategy, List *cn_conns, List *dn_conns)
{
	...
	VACUUM_CLUSTER_DEBUG_LOG((errmsg("relname %s OldestXmin %u, FreezeLimit %u, xidFullScanLimit %u\n",
				RelationGetRelationName(rel), OldestXmin, FreezeLimit, xidFullScanLimit)));
	VACUUM_CLUSTER_DEBUG_LOG((errmsg("vacrel->scanned_pages %u, vacrel->frozenskipped_pages %u, vacrel->rel_pages %u, scanned_all_unfrozen %u\n",
				vacrel->scanned_pages, vacrel->frozenskipped_pages, vacrel->rel_pages, scanned_all_unfrozen)));
...
#ifdef ADB
	if (!IsToastRelation(rel) && (rel->rd_locator_info != NULL))
	{
...
#ifdef ADB
	if (!IsToastRelation(rel) && IsConnFromCoord() && IS_PGXC_DATANODE)
	{
