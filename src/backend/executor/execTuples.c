...
ExecCopyRemoteSlotMinimalTuple(TupleTableSlot *slot)
{
	 ...
	 */
	if (TTS_IS_MINIMALTUPLE(slot) &&
		((MinimalTupleTableSlot*)slot)->mintuple)
		return heap_copy_remote_minimal_tuple(((MinimalTupleTableSlot*)slot)->mintuple);
	if (TTS_IS_HEAPTUPLE(slot) &&
		((HeapTupleTableSlot*)slot)->tuple)
		return remote_minimal_tuple_from_heap_tuple(((HeapTupleTableSlot*)slot)->tuple);
	/*
