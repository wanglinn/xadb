...
#ifdef ADB
			if (IsConnFromCoord() &&
				rt_fetch(resultRelationIndex, rangeTable)->relid == InvalidOid)
			{
	...
	if (erm->markType == ROW_MARK_REFERENCE)
	{
		...
		else
		{
			...
			if (!table_tuple_fetch_row_version(erm->relation,
											   ...
											   SnapshotAny, slot))
				elog(ERROR, "failed to fetch tuple for EvalPlanQual recheck");
			ADB_ONLY_CODE(slot->tts_xcnodeoid = InvalidOid);
			return true;
		}
	}
	else
...
EvalPlanQualStart(EPQState *epqstate, Plan *planTree)
{
	...
	foreach(l, parentestate->es_plannedstmt->subplans)
	{
		...
		subplanstate = ExecInitNode(subplan, rcestate, 0 ADB_ONLY_CODE(|EXEC_FLAG_IN_EPQ));
	 ...
	 */
	epqstate->recheckplanstate = ExecInitNode(planTree, rcestate, 0 ADB_ONLY_CODE(|EXEC_FLAG_IN_EPQ));
