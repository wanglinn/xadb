		...
		for (int i = 0; i < natts; i++)
		{
			if (TupleDescAttr(tupdesc, i)->attgenerated == ATTRIBUTE_GENERATED_STORED)
			{
				...
				if (cmdtype == CMD_UPDATE &&
					...
					!bms_is_member(attno, ExecGetExtraUpdatedCols(resultRelInfo, estate)))
				{
				...
				resultRelInfo->ri_PartitionInfo->pi_RootToPartitionMap)
			{
				...
				if (partrouteinfo->pi_PartitionToRootMap == NULL)
				{
					...
					partrouteinfo->pi_PartitionToRootMap = convert_tuples_by_name(RelationGetDescr(resultRelInfo->ri_RelationDesc),
																				  RelationGetDescr(resultRelInfo->ri_RootResultRelInfo->ri_RelationDesc));
					MemoryContextSwitchTo(oldcontext);
				}

				parentSlot = MakeSingleTupleTableSlot(RelationGetDescr(resultRelInfo->ri_RootResultRelInfo->ri_RelationDesc), &TTSOpsVirtual);
...
#ifdef ADB
		if (!TupIsNull(slot))	/* result initialized to NULL */
#endif
...
ExecDelete(ModifyTableState *mtstate,
		   ...
		   bool changingPart,
		   bool *tupleDeleted,
		   TupleTableSlot **epqreturnslot ADB_ONLY_COMMA_ARG(TupleTableSlot *pslot))
{
	ResultRelInfo *resultRelInfo;
	Relation	resultRelationDesc;
	TM_Result	result;
...
lreplace:;
		...
		if (partition_constraint_failed)
		{
			...
			ExecDelete(mtstate, tupleid, oldtuple, planSlot, epqstate,
					   ...
					   true /* changingPart */ , &tuple_deleted, &epqslot ADB_ONLY_COMMA_ARG(NULL));
		...
		{
			TupleTableSlot *saveSlot = NULL;
			saveSlot = ExecProcNodeDMLInXC(estate, planSlot, slot);

			if (!TupIsNull(saveSlot))
			{
		...
		switch (operation)
		{
			...
			case CMD_DELETE:
				slot = ExecDelete(node, tupleid, oldtuple, planSlot,
								  ...
								  true, node->canSetTag,
								  false /* changingPart */ , NULL, NULL ADB_ONLY_COMMA_ARG(slot));
				break;
			default:
				elog(ERROR, "unknown operation");
				break;
			...
			for (i = 0; i < nplans; i++)
			{
				...
				junkresslot =
					ExecInitExtraTupleSlot(estate, NULL,
										   ADB_ONLY_CODE((mtstate->mt_remoterels[i]) ? &TTSOpsMinimalTuple :)
...
fill_slot_with_oldvals(TupleTableSlot *replace_slot, HeapTupleHeader oldtuphd, Bitmapset *modifiedCols)
{
	 ...
	 */
	ExecForceStoreHeapTuple(newtuple, replace_slot, true);
	return replace_slot;
}
#endif
