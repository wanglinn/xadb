...
static int pgxc_node_begin(int conn_count,
...
RequestTypeAsString(RequestType type)
{
	switch(type)
	{
		CASESTR(REQUEST_TYPE_NOT_DEFINED);
		CASESTR(REQUEST_TYPE_COMMAND);
		CASESTR(REQUEST_TYPE_QUERY);
		CASESTR(REQUEST_TYPE_COPY_IN);
		CASESTR(REQUEST_TYPE_COPY_OUT);
		default:
			break;
	}
...
ResponseResultAsString(int result)
{
	switch(result)
	{
		CASESTR(RESPONSE_EOF);
		CASESTR(RESPONSE_COMPLETE);
		CASESTR(RESPONSE_SUSPENDED);
		CASESTR(RESPONSE_TUPDESC);
		CASESTR(RESPONSE_DATAROW);
		CASESTR(RESPONSE_COPY);
		CASESTR(RESPONSE_BARRIER_OK);
		default:
			break;
	}
 ...
 */
static int
pgxc_node_begin(int conn_count,
				PGXCNodeHandle **connections,
				bool need_tran_block,
				bool readOnly,
				char node_type)
 ...
 */
static void
pgxc_node_remote_commit(const char *gid, bool missing_ok)
{
	int					i, result = 0;
 ...
 */
static void
pgxc_node_remote_abort(const char *gid, bool missing_ok)
{
	int				i, result = 0;
	int				write_conn_count = remoteXactState.numWriteRemoteNodes;
	int				read_conn_count = remoteXactState.numReadRemoteNodes;
...
pgxcNodeCopyBegin(const char *query, List *nodelist, Snapshot snapshot, char node_type)
{
	...
	/*
	 * start transaction on AGTM by coordinator,
	 * neither datanode nor other coordinator.
	 */
	if (need_tran_block)
	...
	if (pgxc_node_begin(conn_count, connections, need_tran_block, false, node_type))
	{
...
static ExecNodes *
reget_exec_nodes_by_en_expr(RemoteQueryState *planstate,
							ExecNodes *exec_nodes)
{
	ExprState *estate;
	bool isnull;
	...
	if (IsRelationDistributedByUserDefined(rel_loc_info))
	{
		...
		(void)get_func_signature(rel_loc_info->funcid, &argtypes, &nargs);
		Assert(nelems == nargs);
	}
	...
	foreach (lc, exec_nodes->en_expr)
	{
		...
		if (IsRelationDistributedByUserDefined(rel_loc_info) && en_expr_node)
		{
	...
	result = GetRelationNodes(rel_loc_info,
							  nelems,
							  en_expr_values,
							  en_expr_nulls,
							  en_expr_types,
									...
									RemoteQueryState *remotestate)
{
	...
	{
		if (pgxc_node_send_query_tree(connection, step->sql_statement, step->sql_node) != 0)
			return false;
	}
	return true;
}
...
do_query(RemoteQueryState *node)
{
	...
	PG_TRY();
	{
		...
		if (primaryconnection)
		{
			if (pgxc_node_begin(1, &primaryconnection, need_tran_block,
		...
		{
			if (pgxc_node_begin(1, &connections[i], need_tran_block,
						is_read_only, PGXC_NODE_DATANODE))
				ereport(ERROR,
						(errcode(ERRCODE_INTERNAL_ERROR),
...
ExecRemoteUtility(RemoteQuery *node)
{
	...
	if (exec_type == EXEC_ON_ALL_NODES || exec_type == EXEC_ON_DATANODES)
	{
		if (pgxc_node_begin(dn_conn_count, pgxc_connections->datanode_handles,
				need_tran_block, false, PGXC_NODE_DATANODE))
			ereport(ERROR,
					(errcode(ERRCODE_INTERNAL_ERROR),
					 errmsg("Could not begin transaction on Datanodes")));
	...
	if (exec_type == EXEC_ON_ALL_NODES || exec_type == EXEC_ON_COORDS)
	{
		if (pgxc_node_begin(co_conn_count, pgxc_connections->coord_handles,
					need_tran_block, false, PGXC_NODE_COORDINATOR))
			ereport(ERROR,
					(errcode(ERRCODE_INTERNAL_ERROR),
...
int
pgxcGetInvolvedRemoteNodes(Oid **nodeIds)
{
	return pgxcGetInvolvedNodes(false, nodeIds);
}

int
pgxcGetInvolvedNodes(bool localNode, Oid **nodeIds)
{
	int				i = 0;
	int				count = 0;
	ListCell		*lc = NULL;
	...
	count = list_length(XactWriteNodes);
	if (localNode)
		count += 1;
	...
	*nodeIds = (Oid *)palloc0(count * sizeof(Oid));

	if (localNode)
		(*nodeIds)[i++] = PGXCNodeOid;

	foreach (lc, XactWriteNodes)
	{
		conn = (PGXCNodeHandle *)lfirst(lc);
		(*nodeIds)[i++] = conn->nodeoid;
	}
...
void
init_RemoteXactStateByNodes(int node_cnt, Oid *nodeIds, bool isPrepared)
{
	int		i;
	Oid		nodeOid;
	char	nodeTyp;
	int		nodeIdx;
	bool 	isLocalNodePrepared = false;
	List	*nodelist = NIL;
	List	*coordlist = NIL;
	PGXCNodeAllHandles *pgxc_handles = NULL;

	if (node_cnt <= 0 || nodeIds == NULL)
		return ;
	...
	for (i = 0; i < node_cnt; i++)
	{
		...
		{
			isLocalNodePrepared = true;
			continue;
		}

		nodeTyp = get_pgxc_nodetype(nodeOid);
		nodeIdx = PGXCNodeGetNodeId(nodeOid, nodeTyp);
		Assert(nodeIdx >= 0);
		switch (nodeTyp)
		{
			...
			case PGXC_NODE_DATANODE:
				nodelist = lappend_int(nodelist, nodeIdx);
				break;
			default:
				Assert(0);
				break;
	 ...
	 * of commit/abort.
	 */
	init_RemoteXactState(isLocalNodePrepared);

	if (isPrepared && remoteXactState.numWriteRemoteNodes > 0)
	{
...
{
	if (list_length(XactWriteNodes) >= 1 || localWrite)
	{
...
void
TellRemoteXactLocalPrepared(bool status)
{
