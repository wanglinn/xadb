...
void AlterNodeExpansionWork(AlterNodeStmt *stmt, ParseState *pstate)
{
	...
	TableScanDesc	scan_clean = table_beginscan_catalog(rel_clean, 0, NULL);
	HeapTuple		tuple;
...
void AlterNodeExpansionClean(AlterNodeStmt *stmt, struct ParseState *pstate)
{
	...
	TableScanDesc	scan = table_beginscan_catalog(rel_clean, 0, NULL);
...
clean_rel_end_:
		/* when scan is not HeapScanDesc, function heap_scan will report an error */
		LockBufferForCleanup(((HeapScanDesc)scan)->rs_cbuf);
		finish_clean_rel(rel_clean, &tuple->t_self, ((HeapScanDesc)scan)->rs_cbuf);
		LockBuffer(((HeapScanDesc)scan)->rs_cbuf, BUFFER_LOCK_UNLOCK);
	}
	pfree(msg.data);
