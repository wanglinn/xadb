...
PgxcNodeAlterLocal(AlterNodeStmt *stmt)
{
		...
		strcmp(new_node_name, node_name) != 0)
	{
		...
		new_record[Anum_pgxc_node_name - 1] = DirectFunctionCall1(namein, CStringGetDatum(new_node_name));
		new_record_repl[Anum_pgxc_node_name - 1] = true;
		new_record_nulls[Anum_pgxc_node_name - 1] = false;
	}
