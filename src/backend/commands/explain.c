...
ExplainQuery(ParseState *pstate, ExplainStmt *stmt,
			 ...
			 ADB_ONLY_COMMA_ARG(bool isTopLive))
{
...
#ifdef ADB
						CURSOR_OPT_PARALLEL_OK | (es->isTopLive ? CURSOR_OPT_CLUSTER_PLAN_SAFE:0), ctas->into, es,
#else
			...
			ExplainState *es)
{
	...
	if (es->plan_id)
	{
		...
		{
			ExplainPropertyInteger("Plan id", NULL, plan->plan_node_id, es);
		}
	}
#endif
	...
	if(es->analyze && es->verbose && planstate->list_cluster_instrument != NIL)
	{
		...
		foreach(lc, planstate->list_cluster_instrument)
		{
			...
			}else
			{
				...
				ExplainPropertyInteger("Oid", NULL, ci->nodeOid, es);
			}
			...
			{
				es->indent++;
				show_buffer_usage(es, &ci->instrument[0].bufusage, false);
				es->indent--;
			}
			opened_group = false;
			es->indent++;
			for(i=1;i<=ci->num_workers;++i)
			{
				...
				else
				{
					...
					ExplainPropertyInteger("Worker Number", NULL, i-1, es);
