...
#ifdef ADB
#define AT_PASS_DISTRIB			11	/* Redistribution pass */
	...
	foreach(cell, stmt->relations)
	{
		...
		if (list_length(rel->rd_auxlist) > 0)
		{
			...
			foreach (auxcell, rel->rd_auxlist)
			{
				...
				rel = table_open(auxrelid, AccessExclusiveLock);
				rels = lappend(rels, rel);
...
truncate_check_rel(Oid relid, Form_pg_class reltuple)
{
	 ...
	 */
	if (IsConnFromApp() && !enable_aux_dml && RelationIdIsAuxiliary(relid))
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
		  ...
		  AlterTableUtilityContext *context)
{
	...
	ATAuxTableInternal(wqueue, rel, cmd, recurse, lockmode, context);
#endif
			...
			case AT_SetNotNull:
				{
					...
					if (OidIsValid(auxrelid))
					{
						...
						ATPrepCmd(wqueue, auxrel, cmd, false, true, lockmode, context);
...
ATCheckCmd(Relation rel, AlterTableCmd *cmd)
{
	...
	switch (cmd->subtype)
	{
		...
		case AT_DistributeBy:	/* DISTRIBUTE BY ... */
			...
			parent_oid = get_partition_parent_ext(RelationGetRelid(rel), false, false);
...
BuildRedistribCommands(Oid relid, List *subCmds)
{
	...
	foreach(item, subCmds)
	{
		...
		if (cmd->subtype != AT_DistributeBy &&
			OidIsValid(get_partition_parent_ext(RelationGetRelid(rel), false, false)) &&
...
DistribRewriteCatalogs(AlterTableCmd *cmd, Oid childOID, LOCKMODE lockmode)
{
	...
	rel = table_open(childOID, lockmode);
...
checkRedistribution(Relation rel, AlterTableCmd *cmd)
{
	...
	parent_oid = get_partition_parent(RelationGetRelid(rel), false);
