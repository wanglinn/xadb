	   ...
	   BufferAccessStrategy bstrategy, bool isTopLevel)
{
	 ...
	 */
#ifdef ADB
	if ((params->options & VACOPT_IN_CLUSTER) == 0)
#endif /* ADB */
	...
	if (params->options & VACOPT_IN_CLUSTER)
		relations = NIL;	/* waiting coordinator send relations */
	else
#endif /* ADB */
	if (relations != NIL)
	{
	...
	else
	{
		...
		if (IsAutoVacuumWorkerProcess())
			...
			use_own_xacts = false;
#ifdef ADB
		else if (params->options & VACOPT_IN_CLUSTER)
			use_own_xacts = true;
#endif /* ADB */
...
#ifdef ADB
	if ((params->options & VACOPT_IN_CLUSTER) || !under_agtm || IS_PGXC_DATANODE)
		list_datanode = NIL;
	else
		list_datanode = get_vacuum_all_node(relations,
											vac_context,
											(params->options & VACOPT_VACUUM) ? true:false);
					...
					ADB_ONLY_COMMA_ARG(VacuumSyncInfo *vsi))
{
	...
	if (TransactionIdIsNormal(frozenxid) &&
		...
		(TransactionIdPrecedes(pgcform->relfrozenxid, frozenxid) ||
		 ...
		 ADB_ONLY_CODE(|| !IsPostmasterEnvironment)))
	{
		pgcform->relfrozenxid = frozenxid;
		dirty = true;
	}
...
vacuum_rel(Oid relid, RangeVar *relation, VacuumParams *params)
{
	...
	if (params->options & VACOPT_IN_CLUSTER)
	{
		statusFlags |= PROC_IS_CLUSTER_VACUUM;
		if (params->options & VACOPT_CLUSTER_VACUUM_TOAST)
			statusFlags |= PROC_IS_CLUSTER_VACUUM_TOAST;
	}
#endif /* ADB */
	...
	if (!(params->options & VACOPT_FULL))
	{
		...
		ADB_ONLY_CODE(MyProc->statusFlags |= statusFlags);
		ProcGlobal->statusFlags[MyProc->pgxactoff] = MyProc->statusFlags;
		LWLockRelease(ProcArrayLock);
		ADB_ONLY_CODE(statusFlags = 0);
	}
#ifdef ADB
	if (statusFlags != 0)
	{
		LWLockAcquire(ProcArrayLock, LW_EXCLUSIVE);
		MyProc->statusFlags |= statusFlags;
		ProcGlobal->statusFlags[MyProc->pgxactoff] = MyProc->statusFlags;
		LWLockRelease(ProcArrayLock);
	}
#endif /* ADB */
		 ...
		 rel->rd_rel->relkind == RELKIND_MATVIEW))
	{
		...
		if (rel->rd_locator_info != NULL)
			dn_oids = list_union_oid(dn_oids, rel->rd_locator_info->nodeids);
		...
		if (cn_oids != NIL || dn_oids != NIL)
		{
			...
			if (rel->rd_rel->relkind != RELKIND_MATVIEW && dn_oids != NIL)
			{
				foreach(lc, dn_oids)
				{
					PGconn *conn = PQNFindConnUseOid(lfirst_oid(lc));
	...
	else
	{
		...
		if (rel->rd_tableam == GetHeapamTableAmRoutine())
			lazy_vacuum_rel_ext(rel, params, vac_strategy, cn_conns, dn_conns);
		else
			ereport(ERROR,
					errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					errmsg("vacuum only support heap yet!"));
#else
...
static List* get_vacuum_all_node(List *relations, MemoryContext context, bool need_vacuum)
{
	...
	Assert(relations == NIL || IsA(relations, List));

	foreach(lc, relations)
	{
		vrel = lfirst_node(VacuumRelation, lc);
		if (!OidIsValid(vrel->oid))
			ereport(ERROR,
					(errcode(ERRCODE_INTERNAL_ERROR),
					 errmsg("Invalid OID for vacuum \"%s\"", vrel->relation->relname)));
		list = get_vacuum_rel_node(list, vrel->oid, need_vacuum);
	}
...
static int process_master_vacuum_cmd(ClusterVacuumCmdContext *context, const char *data, int len)
{
	...
	case CLUSTER_VACUUM_CMD_VACUUM:
		...
		vacuum_rel(InvalidOid, range, context->params);
	...
	case CLUSTER_VACUUM_CMD_ANALYZE_FORCE_INH:
		namespace = load_node_string(&buf, false);
		relname = load_node_string(&buf, false);
		va_cols = castNode(List, loadNode(&buf));
		range = makeRangeVar(namespace, relname, -1);
		/*
		 * If using separate xacts, start one for analyze. Otherwise,
		 * we can use the outer transaction.
		...
		if (mtype == CLUSTER_VACUUM_CMD_ANALYZE_FORCE_INH)
			params.options |= VACOPT_ANALYZE_FORCE_INH;
		analyze_rel(relid, range, &params, va_cols, context->in_outer_xact, vac_strategy);
		if (context->use_own_xacts)
		{
			PopActiveSnapshot();
			CommitTransactionCommand();
