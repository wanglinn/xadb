...
#ifdef ADB
static int process_master_cluster_cmd(int *options, const char *data, int len)
{
	char *namespace;
	char *relname;
	char *indexname;
...
void cluster_cluster(struct StringInfoData *msg)
{
	...
	buf.cursor = 0;
	buf.len = buf.maxlen;
	options = pq_getmsgint(&buf, sizeof(options));

	SimpleNextCopyFromNewFE((SimpleCopyDataFunction)process_master_cluster_cmd, &options);
}
...
static void send_cluster_cluster_function(List *node_list, int flags, int options)
{
	List *list_conns;
	StringInfoData buf;
	...
	pq_sendint(&buf, options, sizeof(options));
...
cluster(ClusterStmt *stmt, bool isTopLevel)
{
	...
	if (stmt->relation != NULL)
	{
		...
		if (list_table_conns)
		{
			...
			foreach(lc, list_table_conns)
				PQputCopyEnd(lfirst(lc), NULL);

			PQNListExecFinish(list_table_conns, NULL, &PQNFalseHookFunctions, true);
