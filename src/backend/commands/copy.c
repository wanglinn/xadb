		  ...
		  List *options ADB_ONLY_COMMA_ARG(bool cluster_safe))
{
...
#ifdef ADB
	if (raw_query && raw_query->stmt && IsA(raw_query->stmt, Query))
	{
		...
		plan_stmt = pg_plan_query((Query*)raw_query->stmt, cluster_safe ? CURSOR_OPT_CLUSTER_PLAN_SAFE : CURSOR_OPT_PARALLEL_OK, NULL);
	...
	else
	{
		...
		/* plan the query */
#ifdef ADB
		plan = pg_plan_query(query, (cluster_safe ? CURSOR_OPT_CLUSTER_PLAN_SAFE:0)|CURSOR_OPT_PARALLEL_OK, NULL);
#else
		...
		if (!skip_tuple)
		{
			...
			else
			{
				...
				if (resultRelInfo->ri_RelationDesc->rd_att->constr &&
					...
					ExecComputeStoredGenerated(estate, myslot ADB_SEQ_ROWID_COMMA_ARGS(true));
			  ...
			  List *options)
{
	...
	cstate = BeginCopy(pstate, true, rel, NULL, InvalidOid, attnamelist, options ADB_ONLY_COMMA_ARG(false));
	oldcontext = MemoryContextSwitchTo(cstate->copycontext);
...
void DoClusterCopy(CopyStmt *stmt, StringInfo mem_toc)
{
	...
	cstate = BeginCopy(pstate, true, rel, NULL, InvalidOid, NULL, stmt->options, true);
	oldcontext = MemoryContextSwitchTo(cstate->copycontext);
...
static TupleTableSlot* NextLineCallTrigger(CopyState cstate, ExprContext *econtext, void *data)
{
	...
	for(;;)
	{
		...
		if (proute)
		{
			 ...
			 */
			tcmap = resultRelInfo->ri_PartitionInfo->pi_RootToPartitionMap;
			if (tcmap &&
				(cstate->transition_capture != NULL ||
				 (resultRelInfo->ri_TrigDesc &&
				  resultRelInfo->ri_TrigDesc->trig_insert_before_row)))
			{
				slot = execute_attr_map_slot(resultRelInfo->ri_PartitionInfo->pi_RootToPartitionMap->attrMap,
											 relslot,
