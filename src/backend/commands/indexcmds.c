			...
			bool quiet)
{
	...
	if (likely(!IsBootstrapProcessingMode()) && IS_PGXC_COORDINATOR)
	{
		List *indexAttrs = NIL;
		...
		for (i = 0; i < indexInfo->ii_NumIndexAttrs; i++)
		{
			...
			if (indexInfo->ii_IndexAttrNumbers[i] > 0)
				indexAttrs = lappend_int(indexAttrs, indexInfo->ii_IndexAttrNumbers[i]);
		}
...
#warning TODO send ReIndex command to remote if coordinator
#endif /* ADB */
...
#ifdef ADB
		if (IsCnMaster() &&
			(list_datanode = get_reindex_rel_datanode(NIL, relid)) != NIL)
		{
			list_coordinator = GetAllCnIDL(false);
			list_all = list_union_oid(list_coordinator, list_datanode);

			send_cluster_reindex_function(list_all, EXEC_CLUSTER_FLAG_NOT_START_TRANS, params->options);
			...
			list_free(list_conns);
			list_free(list_coordinator);
			list_free(list_datanode);
		}
#endif /* ADB */
