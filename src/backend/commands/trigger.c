	...
	for (i = 0; i < trigdesc->numtriggers; i++)
	{
		...
		else if (newtuple != oldtuple)
		{
			...
			if (rel_locinfo && IsRelationDistributedByValue(rel_locinfo))
				pgxc_check_distcol_update(oldtuple, newtuple, newslot->tts_tupleDescriptor, rel_locinfo);
#endif
				   ...
				   TupleTableSlot **newSlot)
{
	...
	else
	{
		...
		ADB_ONLY_CODE(oldslot->tts_xcnodeoid = PGXCNodeOid);
	}
...
static void pgxc_ar_doadd(TupleTableSlot *slot, ARTupInfo *rs_tupinfo);
static void pgxc_ARAddRow(TupleTableSlot *oldslot, TupleTableSlot *newslot, bool is_deferred);
...
#ifdef ADB
		{
			/* fix: Access to field 'next' results in a dereference of a null
			 * pointer (loaded from field 'tail')
			 */
			AssertArg(events->tail);
 ...
 */
static void
pgxc_ar_doadd(TupleTableSlot *slot, ARTupInfo *rs_tupinfo)
{
	if (!rs_tupinfo->tupstate)
		pgxc_ar_init_tupinfo(rs_tupinfo);

	tuplestore_puttupleslot(rs_tupinfo->tupstate, slot);
 ...
 */
static void
pgxc_ARAddRow(TupleTableSlot *oldslot, TupleTableSlot *newslot, bool is_deferred)
{
	int					query_index = afterTriggers.query_depth;
	ARRowStore		   *rowstore;
	TupleTableSlot	   *firstslot;
	TupleTableSlot	   *secondslot;
	...
	Assert(!TupIsNull(oldslot) || !TupIsNull(newslot));
	 ...
	 */
	if (TupIsNull(oldslot) || TupIsNull(newslot))
	{
		firstslot = (!TupIsNull(oldslot) ? oldslot : newslot);
		secondslot = NULL;
	}
	else
	{
	...
	if (!TupIsNull(firstslot))
		pgxc_ar_doadd(firstslot, &rowstore->rs_tupinfo[0]);
	if (!TupIsNull(secondslot))
		pgxc_ar_doadd(secondslot, &rowstore->rs_tupinfo[1]);
}
