...
static FormData_pg_attribute a8 = {
	...
	true, 'p', 'i', true, false, false, '\0', false, true, 0
		...
		static FormData_pg_attribute a9 = {
			0, {"rowid"}, ORACLE_RIDOID, 0, RID_DATA_SIZE,
			ADB_RowIdAttributeNumber, 0, -1, -1,
			false, 'p', 'i', true, false, false, '\0', false, true, 0
		};
						  ...
						  TupleDesc descriptor)
{
	...
	foreach (cell, funcargs)
	{
		...
		} else
		{
			local_var = makeVar(1,
								local_attnum,
								TupleDescAttr(descriptor, local_attnum - 1)->atttypid,
								TupleDescAttr(descriptor, local_attnum - 1)->atttypmod,
								TupleDescAttr(descriptor, local_attnum - 1)->attcollation,
								0);
