...
foreach my $row (@{ $catalog_data{pg_namespace} })
{
	...
	$namespacenames{ $row->{oid} } = $row->{nspname};
}
...
# ADB_BEGIN
	...
	if (defined $row->{oprnamespace} && $row->{oprnamespace} ne 'pg_catalog')
	{
		$key = sprintf "%s.%s(%s,%s)",
		  $row->{oprnamespace}, $row->{oprname}, $row->{oprleft}, $row->{oprright};
		$operoids{$key} = $row->{oid};
	}
# ADB_END
}
...
# ADB_BEGIN
	...
	{
		$prokey = $row->{pronamespace} . '.' . $row->{proname};
		if (defined $procoids{$prokey})
		{
			$procoids{$prokey} = 'MULTIPLE';
		}
...
# ADB_BEGIN
	...
	if (defined $row->{typnamespace} && $row->{typnamespace} ne 'pg_catalog')
	{
		my $key = sprintf "%s.%s", $row->{typnamespace}, $row->{typname};
...
#ADB_BEGIN
			if (defined $symbol)
			{
				if (defined $bki_values{typnamespace} and $bki_values{typnamespace} ne $namespaceoids{'pg_catalog'})
				{
					$bki_values{oid_symbol} = uc $namespacenames{ $bki_values{typnamespace} } . "_$symbol";
				}else
				{
					$bki_values{oid_symbol} = $symbol;
				}
