...
# ADB_BEGIN
	...
	{
		my $macro = length($arg) > 2 ? substr($arg, 2) : shift @ARGV;
...
my $PG_ORACLE_NAMESPACE = Catalog::FindDefinedSymbolFromData($catalog_data{pg_namespace}, 'PG_ORACLE_NAMESPACE')
							if ($enable_grammar_oracle eq 'yes');
my $PG_MANAGER_NAMESPACE = Catalog::FindDefinedSymbolFromData($catalog_data{pg_namespace}, 'PG_MANAGER_NAMESPACE')
							if ($build_manager eq 'yes');
...
# ADB_BEGIN
	# let type support namespace.op(l,r)
	if (defined $row->{oprnamespace} && $row->{oprnamespace} ne 'PGNSP')
	{
		$key = sprintf "%s.%s(%s,%s)",
		  oid2nsp_name($row->{oprnamespace}), $row->{oprname}, $row->{oprleft}, $row->{oprright};
		$operoids{$key} = $row->{oid};
	}
# ADB_END
}
...
# ADB_BEGIN
	# support namespace.proc
	if (defined $row->{pronamespace})
	{
		$prokey = oid2nsp_name($row->{pronamespace}) . '.' . $row->{proname};
		if (defined $procoids{$prokey})
		{
			$procoids{$prokey} = 'MULTIPLE';
		}
...
# ADB_BEGIN
	...
	{
		my $key = sprintf "%s.%s", oid2nsp_name($row->{typnamespace}), $row->{typname};
...
# ADB_END
		...
		foreach my $column (@$schema)
		{
			...
			$bki_values{$attname} =~ s/\bORANSP\b/$PG_ORACLE_NAMESPACE/g
				if (defined $PG_ORACLE_NAMESPACE);
			$bki_values{$attname} =~ s/\bMGRNSP\b/$PG_MANAGER_NAMESPACE/g
				if (defined $PG_MANAGER_NAMESPACE);
...
#ADB_BEGIN
			if (defined $symbol)
			{
				if (defined $bki_values{typnamespace} and $bki_values{typnamespace} ne $PG_CATALOG_NAMESPACE)
				{
					$bki_values{oid_symbol} = uc oid2nsp_name($bki_values{typnamespace}) . "_$symbol";
				}else
				{
					$bki_values{oid_symbol} = $symbol;
				}
...
{
	my $oid = shift;
	$oid =~ s/\bPGNSP\b/$PG_CATALOG_NAMESPACE/g;
	$oid =~ s/\bORANSP\b/$PG_ORACLE_NAMESPACE/g
		if (defined $PG_ORACLE_NAMESPACE);
	$oid =~ s/\bMGRNSP\b/$PG_MANAGER_NAMESPACE/g
		if (defined $PG_MANAGER_NAMESPACE);

	if ($oid eq "$PG_CATALOG_NAMESPACE")
	{
		return "pg_catalog";
	}elsif (defined($PG_ORACLE_NAMESPACE) and $oid eq "$PG_ORACLE_NAMESPACE")
	{
		return "oracle";
	}elsif (defined($PG_MANAGER_NAMESPACE) and $oid eq "$PG_MANAGER_NAMESPACE")
	{
		return "adbmgr";
	}
	die "Can not found namespace for oid $oid";
}
