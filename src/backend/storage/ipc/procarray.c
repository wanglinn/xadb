...
#ifdef ADB
		if (likely(!IsBootstrapProcessingMode()))
		{
			...
			if (TransactionIdIsValid(latestXid) && IsConnFromCoord() &&
				!IsGTMNode() && !(proc->statusFlags & PROC_IS_CLUSTER_VACUUM))
			{
...
retry:
	...
	else if (!single_slave_datanode)
	{
		...
		{
			int icount = count;
			snap = GetGlobalSnapshotGxid(snapshot, &xmin, &xmax, &icount, isCatelog);
			count = icount;
			if (!snap)
			{
				LWLockRelease(ProcArrayLock);
	...
	if (!snapshot->takenDuringRecovery)
	{
		...
		for (size_t pgxactoff = 0; pgxactoff < numProcs; pgxactoff++)
		{
			...
			if (statusFlags & (PROC_IN_LOGICAL_DECODING | PROC_IN_VACUUM ADB_ONLY_CODE(|PROC_IS_EXPANSION_WORKER)))
				continue;
			...
			if (!suboverflowed)
			{
				...
				else
				{
					...
					if (nsubxids > 0)
					{
						...
						}else if(nsubxids + subcount > GetMaxSnapshotSubxidCount())
						{
							suboverflowed = true;
						}else
						{
	...
	if (!TransactionIdIsValid(MyProc->xmin))
	{
		...
		if (TransactionIdIsNormal(global_snap_xmin) && NormalTransactionIdPrecedes(global_snap_xmin, xmin))
			MyProc->xmin = TransactionXmin = global_snap_xmin;
		else
#endif
...
#ifdef ADB
	#warning removed RecentGlobalXmin and RecentGlobalDataXmin
...
ProcArrayInstallRestoredXmin(TransactionId xmin, PGPROC *proc)
{
	 ...
	 */
	xid = UINT32_ACCESS_ONCE(proc->xmin);
	if ((proc->databaseId == MyDatabaseId ADB_ONLY_CODE(|| MyProc->statusFlags & PROC_IS_EXPANSION_WORKER)) &&
...
void SerializeActiveTransactionIds(StringInfo buf)
{
	...
	for (index = 0; index < numProcs; ++index)
	{
		...
		TransactionId xid = (TransactionId)(*((volatile TransactionId*)&pgproc->xid));
		if (TransactionIdIsNormal(xid) && pgproc->is_gtm_2pc == false)
		{
			xids[count++] = xid;
...
bool execLocalFinishActiveBackend(void)
{
	...
	foreach (lc, kill_list)
	{
		...
		if (kill(bpid, stop_signal) < 0)
			ereport(ERROR,
				(errmsg("Failed to end process %d: %m", bpid)));
	}
...
getActiveBackendList(void)
{
	...
	for (index = 0; index < numProcs; ++index)
	{
		...
		if (proc->statusFlags & PROC_IS_EXPANSION_WORKER)
			continue;
