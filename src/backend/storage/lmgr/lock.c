...
LOCKMASK
LockGetAllLocalLocked(const LOCKTAG *locktag)
{
	LOCALLOCK  *locallock;
	LOCALLOCKTAG localtag;
	LOCKMASK	mask = 0;
	...
	for(localtag.mode = MaxLockMode;localtag.mode>NoLock;--localtag.mode)
	{
		locallock = hash_search(LockMethodLocalHash, &localtag, HASH_FIND, &found);
		if(found && locallock->nLocks > 0)
			mask |= LOCKBIT_ON(localtag.mode);
	}
	return mask;
}
...
/* refer AtPrepare_Locks and PostPrepare_Locks, before tranfer lock to SnapRcv/GxidSender, do some prepare work 
   NOTE this only for 1PC. 2PC has done this work in AtPrepare_Locks and PostPrepare_Locks*/
void AdbPrepareTransferLock(void)
{
	HASH_SEQ_STATUS status;
	LOCALLOCK  *locallock;
