...
static Expr* make_aux_delete_clause(Relation aux_rel, Index aux_relid, RangeTblEntry *pts_rte, Index pts_relid, bool filter_nulls)
{
	...
	if (filter_nulls)
	{
		...
		null_test = makeNullTest(copyObject(pts_var), IS_NOT_NULL, false, -1);
		equals = lappend(equals, null_test);
	}else
	{
		/* tuplestore.key_name is null */
		null_test = makeNullTest(copyObject(pts_var), IS_NULL, false, -1);
		equals = list_make1(null_test);

		/* aux_rel.key is null */
		null_test = makeNullTest(copyObject(aux_var), IS_NULL, false, -1);
		equals = lappend(equals, null_test);
	}
