		...
		case T_NullTest:
			{
				...
				if (IsOracleParseGram(pstate) &&
					pstate->p_expr_kind == EXPR_KIND_WHERE &&
					TypeCategory(exprType((Node *) n->arg)) == TYPCATEGORY_STRING)
				{
					Expr *auto_expr = NULL;
...
static Node* tryTransformRowidOpExpr(Node *result, ParseState *pstate)
{
	...
	if (result && IsA(result, OpExpr) &&
		((OpExpr*)result)->opno == RowidEqualOperator &&
		(expr = get_rowid_op_expr((OpExpr*)result, &var)) != NULL)
	{
...
static void tryTransformRowidIn(ScalarArrayOpExpr *saop, ParseState *pstate)
{
	...
	if (!IsA(var, Var) ||
		...
		arr->element_typeid != ORACLE_ROWIDOID)
		return;
...
transformCaseExpr(ParseState *pstate, CaseExpr *c)
{
	...
	foreach(l, c->args)
	{
...
#ifdef ADB_EXT
			/* convert arg is null */
			if (IsA(warg, NullTest) &&
				(((NullTest*)warg)->arg == NULL ||
				 ((NullTest*)warg)->arg == c->arg ||
				 equal(((NullTest*)warg)->arg, c->arg)))
			{
				NullTest *nt = copyObject((NullTest*)warg);
				nt->arg = (Expr*)placeholder;
				warg = (Node*)nt;
			}else
#endif /* ADB_EXT */
				...
				((NullTest*)warg)->arg == NULL)
		{
			ereport(ERROR,
					(errcode(ERRCODE_SYNTAX_ERROR),
					 errmsg("syntax error"),
					 parser_errposition(pstate, ((NullTest*)warg)->location)));
		}
#endif /* ADB_EXT */
	...
	if (IsOracleParseGram(pstate) && c->isdecode)
	{
		...
		ptype = select_oracle_type(pstate, resultexprs,
								   "DECODE", NULL,
								   ORA_CONVERT_KIND_SPECIAL_FUN,
								   "decode");
	}else
	{
#endif
