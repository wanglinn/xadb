...
addDefaultDistributeByOfMultiInherit(CreateStmt *stmt)
{
	...
	foreach(entry, stmt->inhRelations)
	{
		...
		parent = table_openrv((const RangeVar *) rv,
							  ShareUpdateExclusiveLock);
		relloc = RelationGetLocInfo(parent);
		if (relloc == NULL)
		{
		...
		table_close(parent, ShareUpdateExclusiveLock);
...
transformCreateStmt(CreateStmt *stmt, const char *queryString ADB_ONLY_COMMA_ARG(Node **transform_stmt))
{
	...
	foreach(elements, stmt->tableElts)
	{
		...
		switch (nodeTag(element))
		{
			...
			case T_TableLikeClause:
				transformTableLikeClause(&cxt, (TableLikeClause *) element ADB_ONLY_COMMA_ARG(&like_temp_found));
				break;
	 ...
	 */
	if (like_temp_found && transform_stmt)
