	...
	if (IsConnFromApp() && !enable_aux_dml &&
		...
		ereport(ERROR,
				(errmsg("permission denied: \"%s\" is an auxiliary table",
				 ...
				 errhint("INSERT/UPDATE/DELETE on the auxiliary table can only be operated "
...
transformRangeFunction(ParseState *pstate, RangeFunction *r)
{
	 ...
	 */
	item = addRangeTableEntryForFunction(pstate,
	...
	if (pstate->p_grammar == PARSE_GRAM_ORACLE &&
		r->alias == NULL &&
		list_length(item->p_rte->eref->colnames) == 1)
	{
		linitial(item->p_rte->eref->colnames) = makeString("column_value");
	}
#endif /* ADB_GRAM_ORA */
						...
						List **namespace)
{
	...
	{
		Node *node = (*pstate->p_pre_from_item_hook)(pstate, n, top_nsitem, namespace,
	...
	else if (IsA(n, JoinExpr))
	{
		...
		if (j->isNatural)
		{
			...
			foreach(lx, l_colnames)
			{
				...
				if (lx == list_head(l_colnames) && /* first column */
					l_nsitem->p_rte->rtekind == RTE_RELATION /* Ordinary relation */)
				{
					Relation rel = relation_open(l_nsitem->p_rte->relid, AccessShareLock);
		...
		res_colindex +=
			extractRemainingColumns(l_nscolumns, ADB_SEQ_ROWID_ARGS_COMMA(l_nsitem->p_rte)
									l_colnames, &l_colnos,
									&res_colnames, &res_colvars,
									res_nscolumns + res_colindex);
		res_colindex +=
			extractRemainingColumns(r_nscolumns, ADB_SEQ_ROWID_ARGS_COMMA(r_nsitem->p_rte)
									r_colnames, &r_colnos,
									&res_colnames, &res_colvars,
									res_nscolumns + res_colindex);
