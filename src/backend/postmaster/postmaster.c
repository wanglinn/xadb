...
#if defined(ADBMGRD)
#define BACKEND_TYPE_ADBMNT		0x0010	/* adb monitor worker process */
...
PostmasterMain(int argc, char *argv[])
{
	 ...
	 */
	while ((opt = getopt(argc, argv, "B:bc:C:D:d:EeFf:h:ijk:lN:nOPp:r:S:sTt:W:-:" ADB_ONLY_CODE("R:"))) != -1)
	{
		switch (opt)
		{
			case 'B':
...
reaper(SIGNAL_ARGS)
{
	...
	while ((pid = waitpid(-1, &exitstatus, WNOHANG)) > 0)
	{
		...
		if (!IsGTMNode() && pid == SnapReceiverPID)
		{
			...
			{
				LogChildExit(WARNING,
							 _("snapshot receiver process"), pid, exitstatus);
				SnapReceiverPID = StartSnapReceiver();
			}else
			{
				HandleChildCrash(pid, exitstatus,
								 _("snapshot receiver process"));
			}
			continue;
		}
#endif /* ADB */
...
StartAdbmonitorWorker(void)
{
	 ...
	 * changes.
	 */
	if (canAcceptConnections(BACKEND_TYPE_ADBMNT) == CAC_OK)
	{
		bn = (Backend *) malloc(sizeof(Backend));
		if (bn)
		{
