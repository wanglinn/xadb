...
#ifdef USE_SEQ_ROWID
	{
		{"default_with_rowids", PGC_USERSET, COMPAT_OPTIONS_CLIENT,
...
static struct config_string ConfigureNamesString[] =
{
...
#ifdef ADB
	{
		{"recovery_target_barrier", PGC_POSTMASTER, WAL_RECOVERY_TARGET,
			gettext_noop("Sets the barrier of the restore point up to which recovery will proceed."),
			NULL
		},
		&recoveryTargetBarrierId,
		"",
		NULL, assign_recovery_target_barrier, NULL
	},
#endif /* ADB */
...
RestoreGUCState(void *gucstate)
{
	...
	for (i = 0; i < num_guc_variables; i++)
	{
		...
		if (strcmp(gconf->name, GUC_AGTM_HOST) != 0 \
			&& strcmp(gconf->name, GUC_AGTM_PORT) != 0)
#endif
...
check_adbmonitor_max_workers(int *newval, void **extra, GucSource source)
{
	if (MaxConnections + *newval + 1 + max_worker_processes +
		autovacuum_max_workers + 1 + max_wal_senders > MAX_BACKENDS)
		return false;
	return true;
}
#endif
...
#ifdef ADB
void assign_recovery_target_barrier(const char *newval, void *extra)
{
	if (recoveryTarget != RECOVERY_TARGET_UNSET &&
