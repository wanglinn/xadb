...
static Node *build_subplan(PlannerInfo *root, Plan *plan, PlannerInfo *subroot,
						   ...
						   bool unknownEqFalse ADB_ONLY_COMMA_ARG(Path *cluster_path));
			 ...
			 Node *testexpr, bool isTopQual)
{
	...
	result = build_subplan(root, plan, subroot, plan_params,
						   ...
						   ADB_ONLY_COMMA_ARG(final_rel->cheapest_replicate_path));
...
{
	for(;lc!=NULL;lc=lnext(root->parse->cteList, lc))
#else
...
static bool SS_finalize_plan_walker(Plan *plan, void *none, PlannerInfo *root)
{
	...
	if (IsA(plan, ClusterGather) ||
		IsA(plan, ClusterMergeGather))
	{
		(void)finalize_plan(root, outerPlan(plan), -1, root->outer_params, NULL);
