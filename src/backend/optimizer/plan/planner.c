...
grouping_planner(PlannerInfo *root, double tuple_fraction)
{
	...
	else
	{
		...
		if (root->parse->in_sub_plan &&
			expr_have_upper_reference((Expr*)root->processed_tlist, root))
		...
		if (parse->commandType != CMD_SELECT)
		{
				...
				is_remote_relation(root, parse->resultRelation))
			{
				...
				}else if (has_auxiliary || path->rows >= 5.0)
				{
					...
					if(IsA(path, GroupResultPath))
					{
						rp = (GroupResultPath*)path;
						rp->quals = lcons(node_oid_eq, rp->quals);
					}else
					{
						rp = create_group_result_path(root, current_rel, path->pathtarget, list_make1(node_oid_eq));
					}
					memcpy(rp, path, sizeof(Path));
					NodeSetTag(rp, T_GroupResultPath);
					...
					List *activeWindows)
{
	...
	foreach(lc, input_rel->cluster_pathlist)
	{
		...
		if (path == input_rel->cheapest_cluster_total_path ||
			...
			HaveOnceWindowAggClusterPath(activeWindows, root->processed_tlist, path))
									   ...
									   List *activeWindows)
{
	...
	context.tlist = root->processed_tlist;
	...
	foreach(lc, activeWindows)
	{
		...
		}else if(CanOnceWindowAggClusterPath(wc, root->processed_tlist, path))
		{
			context.window_pathkeys = make_pathkeys_for_window(root, wc, root->processed_tlist);
			context.new_path = NULL;
			context.wclause = wc;
			if (lnext(activeWindows, lc) ||
				coord_list ||
				partition_list)
	...
	if(partition_list)
	{
		...
		foreach(lc, partition_list)
		{
			...
			if(coord_list || lnext(partition_list, lc))
			...
			{
				Expr *expr = (Expr*)get_sortgroupclause_expr(lfirst(lc2), root->processed_tlist);
			...
			if(generate_modulo == false && generate_hash == false)
			{
				...
				for(;lc!=NULL;lc=lnext(partition_list, lc))
						  ...
						  GroupPathExtraData *extra)
{
...
#ifdef ADB_EXT
			{
				double	hashaggtablesize = estimate_hashagg_tablesize(root, 
																	  cheapest_path,
																	  agg_costs,
																	  dNumGroups);
...
#ifdef ADB
	if(has_cluster_hazard((Node*)grouped_rel->reltarget->exprs, false)
		|| has_cluster_hazard((Node*)havingQual, false)
	...
	if(try_cluster_aggregation)
	{
		...
		bool no_partial = (root->hasNonPartialAggs || root->hasNonSerialAggs);
		...
		gcontext.final_target = grouped_rel->reltarget;
		gcontext.agg_once_costs = agg_costs;
		gcontext.having_quals = havingQual;
		gcontext.final_groups = dNumGroups;
		gcontext.group_pathkeys = root->group_pathkeys;
		gcontext.group_clause = root->parse->groupClause;
		...
		if (!no_partial)
		{
			gcontext.partial_target = make_partial_grouping_target(root,
			...
			gcontext.agg_final_costs = &extra->agg_final_costs;
			if (!extra->partial_costs_set)
			{
				MemSet(gcontext.agg_partial_costs, 0, sizeof(AggClauseCosts));
		...
		{
			gcontext.agg_partial_costs = NULL;
			gcontext.agg_final_costs = NULL;
		}
			...
			no_partial == false)
		{
			...
			}else
			{
				...
				/* save AGGSPLIT_INITIAL_SERIAL paths */
				initial_paths = gcontext.new_paths_list;
				...
				if (gcontext.can_batch)
				{
					...
					/* step 5: pallel reduce and batch final agg */
					foreach(lc, initial_paths)
					{
						subpath = lfirst(lc);
						ReduceInfoListGetStorageAndExcludeOidList(get_reduce_info_list(subpath),
																  &storage_list,
			...
			(extra->flags & GROUPING_CAN_USE_BATCH) != 0)
		{
			...
			foreach(lc, input_rel->cluster_partial_pathlist)
			{
				subpath = lfirst(lc);
				if (CanOnceGroupingClusterPath(gcontext.final_target, subpath))
					create_cluster_batch_grouping_path(root, subpath, &gcontext);
			}
...
#ifdef ADB_GRAM_ORA
			subpath->pathtarget->as_loc_list = scanjoin_target->as_loc_list;
			subpath->pathtarget->expr_loc_list = scanjoin_target->expr_loc_list;
			subpath->pathtarget->expr_len_list = scanjoin_target->expr_len_list;
			subpath->pathtarget->expr_type_list = scanjoin_target->expr_type_list;
#endif /* ADB_GRAM_ORA */
		}
		else
		{
...
static Path* reduce_to_relation_insert(PlannerInfo *root, Index rel_id, Path *path)
{
			 ...
			 loc_info->locatorType == LOCATOR_TYPE_MODULO)
	{
		...
		reduce_info = MakeReduceInfoUsingPathTarget(loc_info, NIL, path->pathtarget, root);
		Assert(reduce_info);
		path = create_cluster_reduce_path(root, path, list_make1(reduce_info), path->parent, NIL);
	}else
	{
...
static Path* try_simple_remote_insert(PlannerInfo *root, Index relid, Path *subpath, List **exec_nodes)
{
	...
	if (IsA(subpath, GroupResultPath) ||
		IsA(subpath, ProjectionPath))
	{
		...
		if (IsA(subpath, GroupResultPath) &&
			((GroupResultPath*)subpath)->subpath == NULL &&
			((GroupResultPath*)subpath)->quals == NIL &&
		...
		if (IsA(subpath, ProjectionPath) &&
			...
			IsA(((ProjectionPath*)subpath)->subpath, GroupResultPath) &&
		...
		if (exprs != NIL)
		{
			...
			if (IsRelationReplicated(loc_info))
			{
				...
				if (IsA(subpath, GroupResultPath))
					((GroupResultPath*)subpath)->quals = quals;
				else
	...
	}else if(subpath->pathtype == T_ValuesScan)
	{
		...
		if (not_only_const((Node*)values_rte->values_lists) == false)
		{
			...
			if (IsRelationReplicated(loc_info))
			{
				GroupResultPath *result_path = create_group_result_path(root,
																		subpath->parent,
																		subpath->pathtarget,
			...
			{
				rinfo = MakeReduceInfoUsingPathTarget(loc_info, NIL, subpath->pathtarget, root);
				reduce_expr = CreateExprUsingReduceInfo(rinfo);
...
#warning TODO modify function set_modifytable_path_reduceinfo
	//forboth(lc, modify->resultRelations, lc2, modify->subpaths)
	{
		...
		exclude_item = NIL;
		sub_rlist = get_reduce_info_list(modify->subpath);
		if (sub_rlist != NIL)
		{
...
static int create_cluster_distinct_path(PlannerInfo *root, Path *subpath, void *context)
{
	...
	if (dcontext->can_hash)
	{
		...
		hashentrysize += hash_agg_entry_size(0, subpath->pathtarget->width, 0);
	}
...
static inline double get_cluster_grouping_number(List *rlist, double number)
{
	List *nodes;

	if (IsReduceInfoListByValue(rlist))
	{
		ReduceInfoListGetStorageAndExcludeOidList(rlist, &nodes, NULL);
		number /= list_length(nodes);
		if (number < 1.0)
			number = 1.0;
		list_free(nodes);
	}
...
static inline void save_cluster_grouping_path(CreateGrupingPathsContext *gcontext,
											  Path *path, List *reduce_info_list)
{
	path->reduce_info_list = CopyReduceInfoList(reduce_info_list);
...
static int create_cluster_grouping_path(PlannerInfo *root, Path *subpath, void *context)
{
	...
	List					   *rlist = get_reduce_info_list(subpath);
	...
	if (num_groups > 1.0 &&
		gcontext->split != AGGSPLIT_INITIAL_SERIAL)
		num_groups = get_cluster_grouping_number(rlist, num_groups);
	...
	if (gcontext->can_hash &&
		estimate_hashagg_tablesize(root, subpath, costs, num_groups) < work_mem * 1024L)
	{
		path = (Path*)create_agg_path(root,
									  gcontext->grouped_rel,
									  subpath,
		...
		gcontext->split == AGGSPLIT_INITIAL_SERIAL)
	{
		...
		num_batches = estimate_hashagg_tablesize(root, subpath, costs, num_groups) /
							(work_mem * 1024L);
		++num_batches;
...
{
	CreateGrupingPathsContext  *gcontext = context;
	double						num_groups = gcontext->final_groups;
	const AggClauseCosts	   *costs;
	Path					   *path;
	List					   *rlist = get_reduce_info_list(subpath);
	uint32						num_batches;
	Assert((gcontext->can_hash || gcontext->can_sort) && gcontext->can_batch);
	Assert(subpath->parallel_workers > 0 && subpath->parallel_safe);

	if (gcontext->split == AGGSPLIT_SIMPLE)
	{
		costs = gcontext->agg_once_costs;
	}else if (gcontext->split == AGGSPLIT_FINAL_DESERIAL)
	{
		costs = gcontext->agg_final_costs;
	}else
	{
		/* must simple or final, not initial */
		Assert(false);
		return 0;
	}
	...
	if (gcontext->can_hash &&
		estimate_hashagg_tablesize(root, subpath, costs, num_groups/num_batches) <= work_mem * 1024)
	{
		path = (Path*)create_agg_path(root,
									  gcontext->grouped_rel,
									  subpath,
...
static Path *apply_volatile_tlist_cluster_path(PlannerInfo *root, PathTarget *target, Path *path, RelOptInfo *rel)
{
	...
	}else
	{
		...
		RestrictInfo *ri = make_simple_restrictinfo(root, expr);
		GroupResultPath *result_path = create_group_result_path(root, rel, target, list_make1(ri));
