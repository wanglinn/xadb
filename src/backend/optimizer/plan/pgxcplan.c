...
static List *pgxc_process_having_clause(List *remote_tlist, bool single_node_grouping,
												...
												List **remote_qual);
static List *pgxc_set_en_expr(Oid tableoid, Index resultRelationIndex);
...
pgxc_build_shippable_tlist(List *tlist, List *unshippabl_quals, bool has_aggs)
{
	...
	int						flags = PVC_RECURSE_PLACEHOLDERS;
	List					*unshippable_expr = list_copy(unshippabl_quals);
	List					*aggs_n_vars;
	bool					tmp_has_aggs;

	if (has_aggs)
		flags |= PVC_INCLUDE_AGGREGATES;
									...
									List **unshippable_quals, List **rep_tlist)
{
	...
	*unshippable_quals = NIL;
	...
	tlist = pull_var_clause((Node *)tlist, PVC_RECURSE_PLACEHOLDERS);
	tlist = list_concat(tlist, pull_var_clause((Node *)*unshippable_quals, PVC_RECURSE_PLACEHOLDERS));
	...
	*rep_tlist = add_to_flat_tlist(NIL, tlist);
	...
	tlist = list_concat(pull_var_clause((Node *)query->targetList, PVC_RECURSE_PLACEHOLDERS),
						pull_var_clause((Node *)query->jointree->quals,	PVC_RECURSE_PLACEHOLDERS));
	foreach (lcell, tlist)
	{
									...
									List **unshippable_quals, List **rep_tlist)
{
	...
	*rep_tlist = add_to_flat_tlist(NIL, varlist);
	...
	varlist = list_concat(pull_var_clause((Node *)tlist, PVC_RECURSE_PLACEHOLDERS),
						  pull_var_clause((Node *)join_clauses, PVC_RECURSE_PLACEHOLDERS));
	varlist = list_concat(varlist, pull_var_clause((Node *)other_clauses, PVC_RECURSE_PLACEHOLDERS));
...
{
	List		*var_list = pull_var_clause(node,
...
create_remotequery_plan(PlannerInfo *root, RemoteQueryPath *best_path)
{
	...
	if (root->hasPseudoConstantQuals)
	{
		...
		return pgxc_create_gating_plan(root, (Path *)best_path, (Plan *)result_node, quals);
	}
...
create_remotegrouping_plan(PlannerInfo *root, Plan *local_plan)
{
	...
	if (sort_plan)
	{
		...
		sort_plan = make_sort_from_groupcols(query->groupClause, grpColIdx, (Plan *)remote_scan);
...
pgxc_process_grouping_targetlist(List *local_tlist, bool single_node_grouping)
{
	...
	else
	{
		...
		foreach(temp, local_tlist)
		{
			...
			aggs_n_vars = pull_var_clause(expr,
										  PVC_INCLUDE_AGGREGATES |
										  PVC_RECURSE_PLACEHOLDERS);
	...
	*local_qual = NIL;
	...
	foreach(temp, quals)
	{
		...
		if (!shippable_qual || (!single_node_grouping && has_aggs))
		{
			...
			vars_n_aggs = pull_var_clause(expr,
										  PVC_INCLUDE_AGGREGATES |
										  PVC_RECURSE_PLACEHOLDERS);
...
fetch_ctid_of(Plan *subtree, Query *query)
{
	...
	if (IsA(subtree, RemoteQuery))
	{
		...
		if (OidIsValid(funcid))
		{
			...
			func_expr = makeFuncExpr(funcid, rettype, NULL, InvalidOid, InvalidOid, COERCE_EXPLICIT_CALL); /* Just a tentative fix.  Need Ashutosh's review. K.Suzuki */
...
{
	return AddRemoteParseTree(stmts, queryString, NULL, remoteExecType, is_temp);
}

List *AddRemoteParseTree(List *stmts, const char *queryString, const Node *parseTree,
