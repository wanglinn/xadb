...
build_path_tlist(PlannerInfo *root, Path *path)
{
	...
	foreach(v, path->pathtarget->exprs)
	{
		...
		if (lc_as_loc)
		{
			...
			lc_as_loc = lnext(path->pathtarget->as_loc_list, lc_as_loc);
		}else
		{
			tle->as_location = -1;
		}
		if (lc_expr_loc)
		{
			...
			lc_expr_loc = lnext(path->pathtarget->expr_loc_list, lc_expr_loc);
		}else
		{
			tle->expr_loc = -1;
		}
		if (lc_expr_len)
		{
			...
			lc_expr_len = lnext(path->pathtarget->expr_len_list, lc_expr_len);
		}else
		{
			tle->expr_len = -1;
		}
		if (lc_expr_type)
		{
			...
			lc_expr_type = lnext(path->pathtarget->expr_type_list, lc_expr_type);
		}else
		{
...
static bool find_cluster_reduce_expr(Path *path, List **pplist)
{
	...
	if ((IsA(path, GroupResultPath) &&
		 ((GroupResultPath*)path)->subpath == NULL) ||
...
static ConnectByPlan *create_connect_by_plan(PlannerInfo *root, ConnectByPath *path)
{
	...
	for (i=0,lc=list_head(list);lc!=NULL;lc=lnext(list, lc))
	{
