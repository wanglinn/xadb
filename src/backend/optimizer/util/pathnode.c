...
void add_cluster_path(RelOptInfo *parent_rel, Path *new_path)
{
	 ...
	 * it.
	 */
	foreach(p1, parent_rel->cluster_pathlist)
	{
		Path	   *old_path = (Path *) lfirst(p1);
		bool		remove_old = false; /* unless new proves superior */
		PathCostComparison costcmp;
		...
		if (remove_old)
		{
			parent_rel->cluster_pathlist =
				foreach_delete_current(parent_rel->cluster_pathlist, p1);
	...
	if (accept_new)
	{
		/* Accept the new path: insert it at proper place in pathlist */
		parent_rel->cluster_pathlist = 
			list_insert_nth(parent_rel->cluster_pathlist, insert_at, new_path);
	}
	else
	{
		/* Reject and recycle the new path */
...
void add_cluster_partial_path(RelOptInfo *parent_rel, Path *new_path)
{
	 ...
	 * path, but throw out the new path if some existing path dominates it.
	 */
	foreach(p1, parent_rel->cluster_partial_pathlist)
	{
		Path	   *old_path = (Path *) lfirst(p1);
		bool		remove_old = false; /* unless new proves superior */
		PathKeysComparison keyscmp;
		...
		if (remove_old)
		{
			parent_rel->cluster_partial_pathlist =
				foreach_delete_current(parent_rel->cluster_partial_pathlist, p1);
			pfree(old_path);
		}
		else
		{
	...
	if (accept_new)
	{
		/* Accept the new path: insert it at proper place */
		parent_rel->cluster_partial_pathlist = 
			list_insert_nth(parent_rel->cluster_partial_pathlist, insert_at, new_path);
	}
	else
	{
		/* Reject and recycle the new path */
...
{
	QualCost	qual_cost;
	FilterPath *filter = makeNode(FilterPath);
	...
	case T_ModifyTablePath:
		{
			...
			if (get_path_execute_on_walker(mtpath->subpath, context))
				return true;
		}
		return false;
	...
	case T_IndexPath:
		{
			...
			ListCell  *lc;
			if (restrict_list_have_upper_reference(index->indexinfo->indrestrictinfo, root))
				return true;

			foreach (lc, index->indexclauses)
			{
				IndexClause *ic = lfirst_node(IndexClause, lc);
				if (expr_have_upper_reference(ic->rinfo->clause, root) ||
					restrict_list_have_upper_reference(ic->indexquals, root))
					return true;
			}
		}
		return false;
