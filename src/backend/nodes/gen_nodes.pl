...
my $scalar_ident = 'char|bool|int|int16|uint16|int32|uint32|bits32|double|long|Cost|AttrNumber|Index|Oid|BlockNumber|Selectivity';
...
if (defined $special_file)
{
	...
	while(<H>)
	{
		...
		elsif (/^\s*ENUM_IF_DEFINED\s*\(\s*($ident)\s*,($reg_args)\)\s*$/)
		{
			my $enum_name = $1;
			my $defined_str = $2;
			$defined_str =~ s/\s+//g;
			my @defined_args = split(/,/, $defined_str);
			$enum_if_defined{$enum_name} = \@defined_args;
		}
...
{
	if(defined $enum_if_defined{$key})
	{
		my $tmp_str = "\n#if";
		foreach my $item (@{$enum_if_defined{$key}})
		{
...
foreach my $node_tag (@node_tags)
{
	...
	foreach my $item (@{$all_node{$node_tag}})
	{
		...
		# function point
		elsif ($item =~ /^(const\s+){0,1}(struct\s+){0,1}$ident\s+\(\s*\*\s*($ident)\s*\)\s*\(.*\)$/)
		{
			...
			{
				print H "\tNODE_OTHER_POINT(void,$3)\n";
			}
		}
