 ...
 */
static int
plsql_yylex(const ScanKeywordList *unreserved_kws, const uint16 *unreserved_tokens GRAM_ORA_CODE(, int extern_bracket))
{
	int			tok1;
	TokenAuxData aux1;
	int			kwnum;
...
plpgsql_yylex(void)
{
	return plsql_yylex(&UnreservedPLKeywords, UnreservedPLKeywordTokens GRAM_ORA_CODE(, '['));
}
...
void plorasql_scanner_init(const char *str)
{
	...
	yyscanner = scanner_init(str,
							 &core_yy,
							 &ReservedPLORAKeywords,
							 ReservedPLORAKeywordTokens);
